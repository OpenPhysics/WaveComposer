...
export default class FiniteStatusBar extends StatusBar {
    ...
    constructor(layoutBounds, visibleBoundsProperty, scoreProperty, providedOptions) {
        const options = optionize()({
            ...
            startOverButtonText: string_vegas_startOver_StringProperty,
            clockIconRadius: 15,
            xSpacing: 50,
            xMargin: 20,
            yMargin: 10,
        ...
        assert && assert((options.challengeNumberProperty && options.numberOfChallengesProperty) ||
            (!options.challengeNumberProperty && !options.numberOfChallengesProperty), 'challengeNumberProperty and numberOfChallengesProperty are both or neither');
        ...
        if (options.levelNumberProperty && options.levelNumberVisible) {
            const labelStringProperty = options.levelLabelStringProperty || string_vegas_label_level_StringProperty;
            const levelStringProperty = new DerivedStringProperty([labelStringProperty, options.levelNumberProperty], (pattern, level) => StringUtils.format(pattern, level));
            levelNumberText = new Text(levelStringProperty, combineOptions({
        ...
        if (options.challengeNumberProperty && options.numberOfChallengesProperty) {
            const challengeNumberStringProperty = new DerivedStringProperty([string_vegas_pattern_0challenge_1max_StringProperty, options.challengeNumberProperty, options.numberOfChallengesProperty], (pattern, challengeNumber, numberOfChallenges) => StringUtils.format(pattern, challengeNumber, numberOfChallenges));
        ...
        // Start Over button
        const startOverButton = new TextPushButton(options.startOverButtonText, combineOptions({
            font: options.font,
            textFill: options.textFill,
