...
import { splashScreenDownloadComplete } from './splash.js';
import '../../globals.js';
import _ from 'lodash';
import '../../chipper/js/browser/initialize-globals.js';
import { assertionHooks } from '../../scenerystack/assert.js';
...
export default class Sim extends PhetioObject {
    ...
    constructor(simNameProperty, allSimScreens, providedOptions) {
        const splashScreenProgressBarWidth = splashScreenDownloadComplete(phet.chipper.brand);
        ...
        this.frameCounter = 0;
        // Whether the self has resized since our last updateDisplay()
        ...
        this.resizeAction = new PhetioAction((width, height) => {
            ...
            // set the scale describing the target Node, since scale from self resize is applied to each ScreenView,
        ...
        this.stepSimulationAction = new PhetioAction(dt => {
            ...
            if (self.TWEEN) {
                // @ts-expect-error
                self.TWEEN.update(phet.joist.elapsedTime);
            }
        ...
        this.displayedSimNameProperty = DerivedProperty.deriveAny([
            ...
            string_joist_simTitleWithScreenNamePattern_StringProperty,
            ...
            const titleWithScreenPattern = string_joist_simTitleWithScreenNamePattern_StringProperty;
        ...
        assert && assert(self.phet.joist.launchCalled, 'Sim must be launched using simLauncher, ' +
        ...
        assert && assert(!self.phet.joist.sim, 'Only supports one sim at a time');
        self.phet.joist.sim = this;
        ...
        if (this.preferencesModel.audioModel.supportsSound) {
            soundManager.addSoundGenerator(new ScreenSelectionSoundGenerator(this.selectedScreenProperty, this.homeScreen, { initialOutputLevel: 0.5 }), {
                categoryName: 'user-interface'
            });
        }
        // Make ScreenshotGenerator available globally so it can be used in preload files such as PhET-iO.
        self.phet.joist.ScreenshotGenerator = ScreenshotGenerator;
        ...
        if (this.preferencesModel.audioModel.supportsAnyVoicing) {
            this.voicingToolbar = new VoicingToolbar(this.preferencesModel.audioModel.voicingToolbarEnabledProperty, this.selectedScreenProperty, this.lookAndFeel);
    ...
    finishInit(screens) {
        ...
        // Fit to the self and render the initial scene
        ...
        const resizeListener = () => {
            // Don't resize on self size changes if we are playing back input events.
            // See https://github.com/phetsims/joist/issues/37
            if (!phet.joist.playbackModeEnabledProperty.value) {
                this.resizePending = true;
            }
        };
        self.addEventListener('resize', resizeListener);
        self.addEventListener('orientationchange', resizeListener);
        self.visualViewport && self.visualViewport.addEventListener('resize', resizeListener);
        this.resizeToWindow();
    ...
    hidePopup(popup, isModal) {
        ...
        if (isModal) {
            ...
            if (this.modalNodeStack.length === 0) {
                // After hiding all popups, Voicing becomes enabled for components in the simulation self only if
    ...
    resizeToWindow() {
        ...
        this.resize(self.innerWidth, self.innerHeight); // eslint-disable-line phet/bad-sim-text
    }
    resize(width, height) {
    ...
    // Bound to this.boundRunAnimationLoop so it can be run in self.requestAnimationFrame
    runAnimationLoop() {
        self.requestAnimationFrame(this.boundRunAnimationLoop);
