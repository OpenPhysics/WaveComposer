...
class PhetMenu extends Popupable(Node, 0) {
    constructor(sim, providedOptions) {
        ...
        const itemDescriptors = [
            {
                textStringProperty: string_joist_menuItem_phetWebsite_StringProperty,
                present: isPhETBrand,
            ...
            {
                textStringProperty: string_joist_menuItem_reportAProblem_StringProperty,
                present: isPhETBrand && !isApp,
                shouldBeHiddenWhenLinksAreNotAllowed: true,
                callback: () => {
                    const url = `${'https://phet.colorado.edu/files/troubleshooting/' +
                        '?sim='}${encodeURIComponent(sim.simNameProperty.value)}&version=${encodeURIComponent(`${sim.version} ${phet.chipper.buildTimestamp ? phet.chipper.buildTimestamp : '(unbuilt)'}`)}&url=${encodeURIComponent(self.location.href)}&dependencies=${encodeURIComponent(JSON.stringify({}))}`;
                    openPopup(url);
                }
            },
            {
                ...
                shouldBeHiddenWhenLinksAreNotAllowed: true,
                callback: () => {
                    openPopup(`http://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(self.location.href)}&size=220x220&margin=0`);
                }
            },
            {
                textStringProperty: string_joist_menuItem_getUpdate_StringProperty,
            ...
            {
                textStringProperty: string_joist_menuItem_screenshot_StringProperty,
                ...
                callback: () => {
                    ...
                    if (self.Blob && !!new self.Blob()) {
                        ...
                        const byteChars = self.atob(dataBase64);
                        const byteArray = new self.Uint8Array(byteChars.length);
                        for (let i = 0; i < byteArray.length; i++) {
                            byteArray[i] = byteChars.charCodeAt(i); // need check to make sure this cast doesn't give problems?
                        }
                        const blob = new self.Blob([byteArray], { type: 'image/png' });
            ...
            {
                textStringProperty: string_joist_menuItem_fullscreen_StringProperty,
            ...
            // About dialog button
            {
                textStringProperty: string_joist_menuItem_about_StringProperty,
                present: true,
                shouldBeHiddenWhenLinksAreNotAllowed: false,
