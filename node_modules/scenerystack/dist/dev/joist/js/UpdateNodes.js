...
const UpdateNodes = {
    ...
    createCheckingNode: function (options) {
        ...
        const checkingNode = new HBox(merge({
            ...
            children: [
                ...
                new VoicingText(string_joist_updates_checking_StringProperty, {
    ...
    createUpToDateNode: function (options) {
        return new HBox(merge({
            ...
            children: [
                ...
                new VoicingText(string_joist_updates_upToDate_StringProperty, {
                    font: new PhetFont(options.big ? 16 : 14),
                    fontWeight: options.big ? 'bold' : 'normal'
                })
            ]
    ...
    createOutOfDateAboutNode: function (options) {
        const stringProperty = new DerivedProperty([string_joist_updates_outOfDate_StringProperty, allowLinksProperty], (outOfDateString, allowLinks) => {
            return allowLinks ? `<a href="{{url}}">${outOfDateString}</a>` : outOfDateString;
        });
        const links = { url: updateCheck.updateURL };
    ...
    createOutOfDateDialogNode: function (dialog, ourVersionString, latestVersionString, options) {
        const latestVersionStringProperty = new DerivedProperty([string_joist_updates_newVersionAvailable_StringProperty], string => {
            return StringUtils.format(string, latestVersionString);
        });
        const ourVersionStringProperty = new DerivedProperty([string_joist_updates_yourCurrentVersion_StringProperty], string => {
            return StringUtils.format(string, ourVersionString);
        });
        const getUpdateButton = new TextPushButton(string_joist_updates_getUpdate_StringProperty, {
        ...
        const noThanksButton = new TextPushButton(string_joist_updates_noThanks_StringProperty, {
        ...
        content.addDisposable(getUpdateButton, noThanksButton, latestVersionText, ourVersionText, latestVersionStringProperty, ourVersionStringProperty);
        return content;
    },
    /**
     * "Offline" state node
    ...
    createOfflineNode: function (options) {
        return new HBox(merge({
            ...
            children: [
                ...
                new VoicingText(string_joist_updates_offline_StringProperty, {
                    font: new PhetFont(options.big ? 16 : 14),
                    fontWeight: options.big ? 'bold' : 'normal'
                })
            ]
