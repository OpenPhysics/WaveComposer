...
class SimDisplay extends Display {
    constructor(providedOptions) {
        ...
        // override self.open with a semi-API-compatible function, so fuzzing doesn't open new windows.
        if (phet.chipper.isFuzzEnabled()) {
            // @ts-expect-error - it isn't yet clear how to define objects to self just yet
            self.open = function () {
                return {
                    focus: _.noop,
                    blur: _.noop
        ...
        if (phet.chipper.queryParameters.stringTest === 'dynamic') {
            ...
            self.addEventListener('keydown', event => dynamicStringTest.handleEvent(event));
        }
        ...
        self.phet.joist.rootNode = this.rootNode; // make the scene available for debugging
        self.phet.joist.display = this; // make the display available for debugging
