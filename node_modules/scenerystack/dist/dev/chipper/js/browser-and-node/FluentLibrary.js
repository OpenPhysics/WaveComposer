...
class FluentLibrary {
     ...
     * code.
     */
    static getFluentMessageKeys(fluentFileString) {
     ...
     */
    static verifyFluentFile(fluentFileString) {
        const parser = new FluentParser();
        const resource = parser.parse(fluentFileString);
        ...
        const undefinedTerms = Array.from(collector.usedTermNames).filter(term => !termKeys.includes(term));
        ...
        const undefinedMessages = Array.from(collector.usedMessageNames).filter(messageKey => !allDefinedKeys.includes(messageKey));
        ...
        Array.from(collector.declaredTerms).forEach((term) => {
            if (term && term.value &&
                term.value.elements &&
                term.value.elements.some((element) => element.type === 'Placeable')) {
        ...
        collector.foundJunk.forEach((junk) => {
            const messages = junk.annotations.map((annotation) => annotation.message).join('\n');
...
export class FluentVisitor extends Visitor {
    constructor() {
        super(...arguments);
        this.usedTermNames = new Set();
        this.usedMessageNames = new Set();
        this.usedTerms = new Set();
        this.foundJunk = new Set();
        this.declaredTerms = new Set();
    }
    visitTerm(node) {
