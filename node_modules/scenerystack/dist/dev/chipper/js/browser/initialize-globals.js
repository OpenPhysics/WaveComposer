...
import '../../../babel/babel-strings.js';
import { enableAssert } from '../../../scenerystack/assert.js';
 ...
 */
(function () {
    assert && assert(QueryStringMachine, 'QueryStringMachine is used, and should be loaded before this code runs');
    // Create the attachment point for all PhET globals
    self.phet = self.phet ?? {};
    self.phet.preloads = self.phet.preloads || {};
    ...
    const QUERY_PARAMETERS_SCHEMA = {
        ...
        locale: {
            type: 'string',
            defaultValue: self.phet.chipper.locale ?? FALLBACK_LOCALE
         ...
         * in operation (as determined currently by using Chrome's self.performance), then an error will be thrown.
         *
        ...
        profiler: { type: 'flag' },
        /**
         * adds a menu item that will open a self with a QR code with the URL of the simulation
         */
        qrCode: { type: 'flag' },
        /**
         ...
         * to the self so that it can be used in platforms where it otherwise would not be available.
         * Assumes that an implementation of SpeechSynthesis is available from a parent iframe self.
    ...
    };
    {
        ...
        self.phet.chipper.queryParameters = QueryStringMachine.getAll(QUERY_PARAMETERS_SCHEMA);
        // Are we running a built html file? (Note, if running in a Web Worker, we
        // won't have a document to check for this).
        self.phet.chipper.isProduction = self.document ? self.document.querySelector('meta[name="phet-sim-level"]')?.content === 'production' : true;
        // Are we running in an app?
        self.phet.chipper.isApp = phet.chipper.queryParameters['phet-app'] || phet.chipper.queryParameters['phet-android-app'];
        /**
    ...
    {
        self.phet.chipper.colorProfiles = colorProfiles;
        /**
         ...
         * @returns {boolean}
         */
        self.phet.chipper.isFuzzEnabled = () => (self.phet.chipper.queryParameters.fuzz ||
            self.phet.chipper.queryParameters.fuzzMouse ||
            self.phet.chipper.queryParameters.fuzzTouch ||
            self.phet.chipper.queryParameters.fuzzBoard);
        self.phet.chipper.supportsAnyVoicing = () => {
            return self.phet.chipper.queryParameters.supportsVoicing ||
                self.phet.chipper.queryParameters.supportsCoreVoicing;
        };
        ...
        if (self.phet.chipper.queryParameters.log) {
            self.phet.log = function (message, options) {
                options = _.assignIn({
                    color: '#009900' // green
                }, options);
         ...
         * @returns {string}
         */
        self.phet.chipper.brand = self.phet.chipper.brand || phet.chipper.queryParameters.brand || 'adapted-from-phet';
        ...
        const stringTest = (typeof self !== 'undefined' && phet.chipper.queryParameters.stringTest) ?
         ...
         * @returns {string}
         */
        self.phet.chipper.mapString = function (string) {
            const script = 'script';
     ...
     */
    self.phet.chipper.makeEverythingSlow = function () {
        self.setInterval(() => { sleep(64); }, 16);
    };
    self.phet.chipper.makeRandomSlowness = function () {
        self.setInterval(() => { sleep(Math.ceil(100 + Math.random() * 200)); }, Math.ceil(100 + Math.random() * 200));
    };
    (function () {
        /**
         ...
         */
        self.phet.chipper.reportContinuousTestResult = options => {
            self.parent && self.parent.postMessage(JSON.stringify(_.assignIn({
        ...
        if (phet.chipper.queryParameters.continuousTest) {
            ...
            // self.open stub. otherwise we get tons of "Report Problem..." popups that stall
            self.open = () => {
                return {
                    focus: () => { },
                    blur: () => { }
        ...
        if (phet.chipper.queryParameters.postMessageOnError) {
            self.addEventListener('error', a => {
                ...
                self.parent && self.parent.postMessage(JSON.stringify({
                    type: 'error',
                    url: self.location.href,
                    message: message,
                    stack: stack
