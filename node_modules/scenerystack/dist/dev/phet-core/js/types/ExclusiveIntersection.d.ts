 ...
 */
type ExclusiveIntersection<A, B extends {
    [K in keyof B]: K extends keyof A ? never : B[K];
}, C extends {
    [K in keyof C]: K extends keyof A ? never : K extends keyof B ? never : C[K];
} = {}, // eslint-disable-line @typescript-eslint/no-restricted-types
D extends {
    [K in keyof D]: K extends keyof A ? never : K extends keyof B ? never : K extends keyof C ? never : D[K];
} = {}> = A & B & C & D;
