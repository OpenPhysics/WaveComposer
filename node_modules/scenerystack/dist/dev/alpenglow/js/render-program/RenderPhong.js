...
export class RenderPhong extends RenderProgram {
    constructor(alpha, ambientColorProgram, diffuseColorProgram, specularColorProgram, positionProgram, normalProgram, lights) {
        super([
            ambientColorProgram,
            diffuseColorProgram,
        ...
        this.alpha = alpha;
        this.ambientColorProgram = ambientColorProgram;
        this.diffuseColorProgram = diffuseColorProgram;
        this.specularColorProgram = specularColorProgram;
    ...
    withChildren(children) {
        ...
        return new RenderPhong(this.alpha, children[0], children[1], children[2], children[3], children[4], _.range(0, lightChildren.length / 2).map(i => {
    ...
    getSimplified(children) {
        ...
        if (normalProgram.isFullyTransparent ||
        ...
        else if (ambientColorProgram instanceof RenderColor &&
            ...
            return new RenderColor(this.getPhong(ambientColorProgram.color, diffuseColorProgram.color, specularColorProgram.color, positionProgram.color, normalProgram.color, lightDirectionPrograms.map(program => program.color), lightColorPrograms.map(program => program.color)));
        }
        else {
            return null;
        }
    }
    getPhong(ambientColor, diffuseColor, specularColor, position, normal, lightDirections, lightColors) {
    ...
    evaluate(context) {
        ...
        return this.getPhong(ambientColor, diffuseColor, specularColor, position, normal, lightDirections, lightColors);
    }
    ...
    execute(stack, context, executor) {
        ...
        for (let i = 0; i < this.numLights; i++) {
            ...
            if (dot > 0) {
                ...
                scratchOutputVector.addXYZW(scratchLightColorVector.x * scratchDiffuseVector.x * dot +
                    scratchLightColorVector.x * scratchSpecularVector.x * specularContribution, scratchLightColorVector.y * scratchDiffuseVector.y * dot +
                    scratchLightColorVector.y * scratchSpecularVector.y * specularContribution, scratchLightColorVector.z * scratchDiffuseVector.z * dot +
                    scratchLightColorVector.z * scratchSpecularVector.z * specularContribution, scratchLightColorVector.w * scratchDiffuseVector.w +
                    scratchLightColorVector.w * scratchSpecularVector.w // keep alphas
                );
            }
        }
        // clamp for now
        stack.pushValues(Math.min(1, scratchOutputVector.x), Math.min(1, scratchOutputVector.y), Math.min(1, scratchOutputVector.z), Math.min(1, scratchOutputVector.w));
    }
