...
export class RenderDepthSort extends RenderProgram {
    constructor(items) {
        ...
        super(children, _.every(children, RenderProgram.closureIsFullyTransparent), _.some(children, RenderProgram.closureIsFullyOpaque), false, false, true // NOTE: If we have this (unsplit), we'll want the centroid
        );
        this.items = items;
    }
    getName() {
    ...
    evaluate(context) {
        ...
        for (let i = 0; i < sortedItems.length; i++) {
            ...
            color.setXYZW(blendColor.x + backgroundAlpha * color.x, blendColor.y + backgroundAlpha * color.y, blendColor.z + backgroundAlpha * color.z, blendColor.w + backgroundAlpha * color.w);
        }
        return color;
    }
    isSplittable() {
    ...
    static getProjectionMatrix(near, far, minX, minY, maxX, maxY) {
        const minZ = near;
        const maxZ = far;
        const diffX = maxX - minX;
        const diffY = maxY - minY;
        const diffZ = maxZ - minZ;
        return new Matrix4(2 * minZ / diffX, 0, -(maxX + minX) / diffX, 0, 0, 2 * minZ / diffY, -(maxY + minY) / diffY, 0, 0, 0, maxZ / diffZ, -minZ * maxZ / diffZ, 0, 0, 1, 0);
    }
}
alpenglow.register('RenderDepthSort', RenderDepthSort);
class SortedPartial {
    constructor(face, items // ordered by decreasing depth (similar to eventual order in RenderStack)
    ) {
        this.face = face;
        this.items = items;
    }
