...
import { RenderInstructionLocation, RenderInstructionReturn } from './RenderInstruction.js';
...
const toProgram = (item) => item.program;
export class RenderRadialGradient extends RenderProgram {
    constructor(transform, start, startRadius, end, endRadius, stops, // should be sorted!!
    extend, accuracy, logic) {
        ...
        super(children, _.every(children, RenderProgram.closureIsFullyTransparent), _.every(children, RenderProgram.closureIsFullyOpaque), false, false, accuracy === RenderRadialGradientAccuracy.UnsplitCentroid || accuracy === RenderRadialGradientAccuracy.SplitCentroid || accuracy === RenderRadialGradientAccuracy.SplitAccurate);
        ...
        this.logic = logic || RenderRadialGradientLogic.from(this.transform, this.start, this.startRadius, this.end, this.endRadius, this.stops.map(stop => stop.ratio), this.extend, this.accuracy);
    }
    getName() {
    ...
    transformed(transform) {
        return new RenderRadialGradient(transform.timesMatrix(this.transform), this.start, this.startRadius, this.end, this.endRadius, this.stops.map(stop => new RenderGradientStop(stop.ratio, stop.program.transformed(transform))), this.extend, this.accuracy);
    }
    equalsTyped(other) {
    ...
    split(face) {
        ...
        const radiusToStop = (radius) => {
            return (radius / deltaRadius) - offset;
        };
        const stopToRadius = (ratio) => {
            ...
            const renderableFaces = linearRanges.map((range, i) => {
                ...
                const replacer = (renderProgram) => {
                    ...
                    if (range.startProgram === range.endProgram) {
                        ...
                        return new RenderRadialBlend(blendTransform, startRadius, endRadius, blendAccuracy, range.startProgram.replace(replacer), range.endProgram.replace(replacer));
                    }
                };
