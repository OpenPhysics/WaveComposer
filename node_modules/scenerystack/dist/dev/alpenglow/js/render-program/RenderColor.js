...
import { RenderInstructionPush } from './RenderInstruction.js';
...
export class RenderColor extends RenderProgram {
    constructor(color) {
        super(emptyChildren, color.x === 0 && color.y === 0 && color.z === 0 && color.w === 0, color.w === 1, false, false, false);
        this.color = color;
    ...
    static applyProgram(vector, color) {
        vector.set(color);
    }
    ...
    static sRGBToLinear(sRGB) {
        ...
        return new Vector4(sRGB.x <= 0.0404482362771082 ? sRGB.x / 12.92 : Math.pow((sRGB.x + 0.055) / 1.055, 2.4), sRGB.y <= 0.0404482362771082 ? sRGB.y / 12.92 : Math.pow((sRGB.y + 0.055) / 1.055, 2.4), sRGB.z <= 0.0404482362771082 ? sRGB.z / 12.92 : Math.pow((sRGB.z + 0.055) / 1.055, 2.4), sRGB.w);
    }
    static linearToSRGB(linear) {
        ...
        return new Vector4(linear.x <= 0.00313066844250063 ? linear.x * 12.92 : 1.055 * Math.pow(linear.x, 1 / 2.4) - 0.055, linear.y <= 0.00313066844250063 ? linear.y * 12.92 : 1.055 * Math.pow(linear.y, 1 / 2.4) - 0.055, linear.z <= 0.00313066844250063 ? linear.z * 12.92 : 1.055 * Math.pow(linear.z, 1 / 2.4) - 0.055, linear.w);
    }
    // Oklab is a perceptually uniform color space, which is useful for color blending.
    // https://bottosson.github.io/posts/oklab/
    // returned as (L,a,b,alpha)
    static linearToOklab(linear) {
        ...
        return new Vector4(0.2104542553 * l_ + 0.7936177850 * m_ - 0.0040720468 * s_, 1.9779984951 * l_ - 2.4285922050 * m_ + 0.4505937099 * s_, 0.0259040371 * l_ + 0.7827717662 * m_ - 0.8086757660 * s_, linear.w);
    }
    static oklabToLinear(oklab) {
        ...
        const m = m_ * m_ * m_;
        const s = s_ * s_ * s_;
        return new Vector4(4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s, -1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s, -0.0041960863 * l - 0.7034186147 * m + 1.7076147010 * s, oklab.w);
    }
    static multiplyMatrixTimesColor(matrix, color) {
        return new Vector4(matrix.m00() * color.x + matrix.m01() * color.y + matrix.m02() * color.z, matrix.m10() * color.x + matrix.m11() * color.y + matrix.m12() * color.z, matrix.m20() * color.x + matrix.m21() * color.y + matrix.m22() * color.z, color.w);
    }
    static linearToLinearDisplayP3(color) {
        return RenderColor.multiplyMatrixTimesColor(RenderColor.sRGBToDisplayP3Matrix, color);
    }
    static linearDisplayP3ToLinear(color) {
        return RenderColor.multiplyMatrixTimesColor(RenderColor.displayP3TosRGBMatrix, color);
    }
    static linearDisplayP3ToDisplayP3(linear) {
        return RenderColor.linearToSRGB(linear); // same transfer curve
    }
    static displayP3ToLinearDisplayP3(displayP3) {
     ...
     */
    static gamutMapColor(color, toOklab, fromOklab) {
        ...
        while (highChroma - lowChroma > 1e-4) {
            ...
            clipped = isInColorRange ? mapped : new Vector4(Math.max(0, Math.min(1, mapped.x)), Math.max(0, Math.min(1, mapped.y)), Math.max(0, Math.min(1, mapped.z)), mapped.w);
    ...
    static gamutMapDisplayP3(color) {
        if (RenderColor.isColorInRange(color)) {
            return color;
        }
        else {
     ...
     */
    static gamutMapPremultipliedSRGB(color) {
        if (color.w <= 1e-8) {
            return Vector4.ZERO;
        }
     ...
     */
    static gamutMapPremultipliedDisplayP3(color) {
        if (color.w <= 1e-8) {
            return Vector4.ZERO;
        }
    ...
    static oklabToLinearDisplayP3(oklab) {
        return RenderColor.linearToLinearDisplayP3(RenderColor.oklabToLinear(oklab));
    }
    static linearDisplayP3ToOklab(linearP3) {
        return RenderColor.linearToOklab(RenderColor.linearDisplayP3ToLinear(linearP3));
    }
    static premultiply(color) {
        return new Vector4(color.x * color.w, color.y * color.w, color.z * color.w, color.w);
    }
    static unpremultiply(color) {
        return color.w === 0 ? Vector4.ZERO : new Vector4(color.x / color.w, color.y / color.w, color.z / color.w, color.w);
    }
    static xyYToXYZ(xyY) {
        /**
         ...
         */
        return new Vector3(xyY.x * xyY.z / xyY.y, xyY.z, (1 - xyY.x - xyY.y) * xyY.z / xyY.y);
    }
    static xyToXYZ(xy) {
        return RenderColor.xyYToXYZ(new Vector3(xy.x, xy.y, 1));
    }
    static xyzToLinear(xyz) {
        return RenderColor.multiplyMatrixTimesColor(RenderColor.XYZTosRGBMatrix, xyz);
    }
    static linearToXYZ(linear) {
        return RenderColor.multiplyMatrixTimesColor(RenderColor.sRGBToXYZMatrix, linear);
    }
    static getMatrixRGBToXYZ(redChromaticity, greenChromaticity, blueChromaticity, whiteChromaticity) {
        ...
        const matrixPrime = Matrix3.rowMajor(redPrimeXYZ.x, greenPrimeXYZ.x, bluePrimeXYZ.x, redPrimeXYZ.y, greenPrimeXYZ.y, bluePrimeXYZ.y, redPrimeXYZ.z, greenPrimeXYZ.z, bluePrimeXYZ.z);
        const rgbY = matrixPrime.inverted().timesVector3(whiteXYZ);
        return matrixPrime.timesMatrix(Matrix3.rowMajor(rgbY.x, 0, 0, 0, rgbY.y, 0, 0, 0, rgbY.z));
    }
    static canvasSupportsDisplayP3() {
    ...
    static ratioBlend(zeroColor, oneColor, ratio) {
        const minusRatio = 1 - ratio;
        return new Vector4(zeroColor.x * minusRatio + oneColor.x * ratio, zeroColor.y * minusRatio + oneColor.y * ratio, zeroColor.z * minusRatio + oneColor.z * ratio, zeroColor.w * minusRatio + oneColor.w * ratio);
    }
    serialize() {
        return {
            type: 'RenderColor',
...
RenderColor.sRGBToXYZMatrix = RenderColor.getMatrixRGBToXYZ(sRGBRedChromaticity, sRGBGreenChromaticity, sRGBBlueChromaticity, sRGBWhiteChromaticity);
RenderColor.XYZTosRGBMatrix = RenderColor.sRGBToXYZMatrix.inverted();
RenderColor.displayP3ToXYZMatrix = RenderColor.getMatrixRGBToXYZ(dciP3RedChromaticity, dciP3GreenChromaticity, dciP3BlueChromaticity, sRGBWhiteChromaticity);
RenderColor.XYZToDisplayP3Matrix = RenderColor.displayP3ToXYZMatrix.inverted();
RenderColor.sRGBToDisplayP3Matrix = RenderColor.XYZToDisplayP3Matrix.timesMatrix(RenderColor.sRGBToXYZMatrix);
RenderColor.displayP3TosRGBMatrix = RenderColor.sRGBToDisplayP3Matrix.inverted();
RenderColor.TRANSPARENT = new RenderColor(Vector4.ZERO);
