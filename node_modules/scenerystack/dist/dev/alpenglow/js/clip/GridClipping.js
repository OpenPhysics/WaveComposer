...
export class GridClipping {
    ...
    // @deprecated
    static gridClipInterceptIterate(startPoint, endPoint, minX, minY, maxX, maxY, stepX, stepY, stepWidth, stepHeight, callback) {
        ...
        if (stepWidth === 1 && stepHeight === 1) {
            callback(0, 0, startPoint.x, startPoint.y, endPoint.x, endPoint.y, startPoint, endPoint);
            return;
        }
        // TODO: get rid of these functions (inline)
        const toStepX = (x) => (x - minX) / stepX;
        const toStepY = (y) => (y - minY) / stepY;
        const fromStepX = (x) => x * stepX + minX;
        const fromStepY = (y) => y * stepY + minY;
        ...
        if (lineStepWidth === 1 && lineStepHeight === 1) {
            ...
            callback(minStepX, minStepY, startPoint.x, startPoint.y, endPoint.x, endPoint.y, startPoint, endPoint);
        }
        else {
                ...
                for (let ix = minStepX; ix < maxStepX; ix++) {
                    ...
                    if ((isLessThanMinX || isGreaterThanMaxX) && (isLessThanMinY || isGreaterThanMaxY)) {
                        ...
                        if (hasHorizontal && hasVertical) {
                            ...
                            callback(ix, iy, xFirst ? otherX : cornerX, xFirst ? cornerY : otherY, cornerX, cornerY, null, null);
                            callback(ix, iy, cornerX, cornerY, xFirst ? cornerX : otherX, xFirst ? otherY : cornerY, null, null);
                        }
                        else if (hasHorizontal) {
                            const y = isLessThanMinY ? cellMaxY : cellMinY;
                            callback(ix, iy, startXLess ? cellMinX : cellMaxX, y, startXLess ? cellMaxX : cellMinX, y, null, null);
                        }
                        else if (hasVertical) {
                            const x = isLessThanMinX ? cellMaxX : cellMinX;
                            callback(ix, iy, x, startYLess ? cellMinY : cellMaxY, x, startYLess ? cellMaxY : cellMinY, null, null);
                        }
                    }
                    else {
                        ...
                        assertSlow && assertSlow((Math.abs(endX - startX) < 1e-8 && Math.abs(endY - startY) < 1e-8) ||
                        ...
                        if (needsStartCorner) {
                            callback(ix, iy, startPoint.x < cellCenterX ? cellMinX : cellMaxX, startPoint.y < cellCenterY ? cellMinY : cellMaxY, startX, startY, null, existingStartPoint);
                        }
                        callback(ix, iy, startX, startY, endX, endY, existingStartPoint, existingEndPoint);
                        if (needsEndCorner) {
                            callback(ix, iy, endX, endY, endPoint.x < cellCenterX ? cellMinX : cellMaxX, endPoint.y < cellCenterY ? cellMinY : cellMaxY, existingEndPoint, null);
                        }
                    }
                }
            }
        ...
        for (let ix = roundedMinStepX; ix < roundedMaxStepX; ix++) {
            ...
            for (let iy = 0; iy < minStepY; iy++) {
                ...
                callback(ix, iy, startXLess ? x0 : x1, y, startXLess ? x1 : x0, y, null, null);
            }
            // max-y side
        ...
        for (let iy = roundedMinStepY; iy < roundedMaxStepY; iy++) {
            ...
            for (let ix = 0; ix < minStepX; ix++) {
                ...
                callback(ix, iy, x, startYLess ? y0 : y1, x, startYLess ? y1 : y0, null, null);
            }
            // max-x side
    ...
    static gridClipIterate(startPoint, endPoint, minX, minY, maxX, maxY, stepX, stepY, stepWidth, stepHeight, callback) {
        assertSlow && assertSlow(startPoint.isFinite());
        assertSlow && assertSlow(endPoint.isFinite());
        assertSlow && assertSlow(isFinite(minX) && Number.isInteger(minX));
        assertSlow && assertSlow(isFinite(minY) && Number.isInteger(minY));
                ...
                for (let ix = minStepX; ix < maxStepX; ix++) {
                    ...
                    if (cellMaxX < minXIntercept - 1e-6 || cellMinX > maxXIntercept + 1e-6) {
                        ...
                        if (hasHorizontal && hasVertical) {
                            ...
                            callback(ix, iy, x0, y0, cornerX, cornerY, null, null);
                            assert && assert(x2 !== cornerX || y2 !== cornerY, 'Should not duplicate points');
                            callback(ix, iy, cornerX, cornerY, x2, y2, null, null);
                        }
                        else if (hasHorizontal) {
                            const y = isLessThanMinY ? cellMaxY : cellMinY;
                            callback(ix, iy, startXLess ? cellMinX : cellMaxX, y, startXLess ? cellMaxX : cellMinX, y, null, null);
                        ...
                        if (clipped) {
                            if (needsStartCorner && (startCornerX !== clippedStartPoint.x || startCornerY !== clippedStartPoint.y)) {
                                callback(ix, iy, startCornerX, startCornerY, clippedStartPoint.x, clippedStartPoint.y, null, existingStartPoint);
                            }
                            if (!clippedStartPoint.equals(clippedEndPoint)) {
                                callback(ix, iy, clippedStartPoint.x, clippedStartPoint.y, clippedEndPoint.x, clippedEndPoint.y, existingStartPoint, existingEndPoint);
                            }
                            if (needsEndCorner && (endCornerX !== clippedEndPoint.x || endCornerY !== clippedEndPoint.y)) {
                                callback(ix, iy, clippedEndPoint.x, clippedEndPoint.y, endCornerX, endCornerY, existingEndPoint, null);
                            }
                        }
                        else {
                            if (cellStartXLess !== cellEndXLess && cellStartYLess !== cellEndYLess) {
                                ...
                                const middlePoint = new Vector2(startSame === yGreater ? cellMinX : cellMaxX, yGreater ? cellMaxY : cellMinY);
                                callback(ix, iy, startCornerX, startCornerY, middlePoint.x, middlePoint.y, null, null);
                                callback(ix, iy, middlePoint.x, middlePoint.y, endCornerX, endCornerY, null, null);
                            }
                            else if (startCornerX !== endCornerX || startCornerY !== endCornerY) {
                                callback(ix, iy, startCornerX, startCornerY, endCornerX, endCornerY, null, null);
                            }
                        }
                    }
                }
