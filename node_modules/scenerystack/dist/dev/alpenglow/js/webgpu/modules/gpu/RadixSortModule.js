...
import { getArrayType, U32Add, U32Type } from '../../compute/ConcreteType.js';
import { u32S, wgsl } from '../../wgsl/WGSLString.js';
import { MainRadixHistogramModule } from './MainRadixHistogramModule.js';
import { MainRadixScatterModule } from './MainRadixScatterModule.js';
...
export const getMaxRadixBitsPerInnerPass = (workgroupSize, grainSize) => {
...
export const getRadixBitVectorSize = (workgroupSize, grainSize, bitsPerInnerPass) => {
    const countBitQuantity = Math.ceil(Math.log2(workgroupSize * grainSize));
    const countsPerComponent = Math.floor(32 / countBitQuantity);
...
export class RadixSortModule extends CompositeModule {
    constructor(providedOptions) {
        const options = optionize3()({}, RADIX_SORT_MODULE_DEFAULTS, providedOptions);
        ...
        assert && assert(getMaxRadixBitsPerInnerPass(options.radixWorkgroupSize, options.radixGrainSize) >= options.bitsPerInnerPass, 'Not enough bits for inner radix sort');
        ...
        if (inPlace) {
            ...
            if (iterationCount % 2 === 0) {
                ...
                for (let i = 0; i < iterationCount; i++) {
                    iterationBufferPairs.push(new BufferPair(i % 2 === 0 ? options.input : extraBuffer, i % 2 === 0 ? extraBuffer : options.output));
                }
            }
            else {
                ...
                iterationBufferPairs.push(new BufferPair(options.input, secondExtraBuffer));
                iterationBufferPairs.push(new BufferPair(secondExtraBuffer, extraBuffer));
                for (let i = 2; i < iterationCount; i++) {
                    iterationBufferPairs.push(new BufferPair(i % 2 === 0 ? extraBuffer : options.output, i % 2 === 0 ? options.output : extraBuffer));
                }
            }
        }
        else {
            if (options.canMutateInput) {
                if (iterationCount % 2 === 0) {
                    ...
                    extraSlots.push(extraBuffer);
                    for (let i = 0; i < iterationCount; i++) {
                        iterationBufferPairs.push(new BufferPair(i % 2 === 0 ? (i === 0 ? options.input : options.output) : extraBuffer, i % 2 === 0 ? extraBuffer : options.output));
                    }
                }
                else {
                    ...
                    // extras!
                    for (let i = 0; i < iterationCount; i++) {
                        iterationBufferPairs.push(new BufferPair(i % 2 === 0 ? options.input : options.output, i % 2 === 0 ? options.output : options.input));
                    }
                }
            }
            else {
                ...
                for (let i = 0; i < iterationCount; i++) {
                    iterationBufferPairs.push(new BufferPair(i % 2 === 0 ? (i === 0 ? options.input : evenOutputBuffer) : oddOutputBuffer, i % 2 === 0 ? oddOutputBuffer : evenOutputBuffer));
                }
            }
        }
        const histogramModules = _.range(0, iterationCount).map(i => {
            const bufferSlot = iterationBufferPairs[i].input;
            return new MainRadixHistogramModule(combineOptions({
        ...
        const scatterModules = _.range(0, iterationCount).map(i => {
            ...
            return new MainRadixScatterModule(combineOptions({
        ...
        const scanModule = new ScanModule(combineOptions({
    ...
    static getMaximumElementQuantity(radixWorkgroupSize, radixGrainSize, scanWorkgroupSize, scanGrainSize, bitsPerPass, scanLevels = 3) {
