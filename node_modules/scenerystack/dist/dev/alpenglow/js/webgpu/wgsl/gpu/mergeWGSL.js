...
import { u32S, wgsl, WGSLModule, wgslWith } from '../WGSLString.js';
...
export const mergeWGSL = (providedOptions) => {
    const options = optionize3()({}, MERGE_DEFAULTS, providedOptions);
  ...
  `);
...

    {
      ...
      if ( block_length > 0u ) {
        ...
        compare: compare ? ((indexA, indexB) => compare(wgsl `${workgroupA}[ ( ${indexA} + processed_index_a ) % ${u32S(sharedMemorySize)} ]`, wgsl `${workgroupB}[ ( ${indexB} + processed_index_b ) % ${u32S(sharedMemorySize)} ]`)) : null,
        greaterThan: greaterThan ? ((indexA, indexB) => greaterThan(wgsl `${workgroupA}[ ( ${indexA} + processed_index_a ) % ${u32S(sharedMemorySize)} ]`, wgsl `${workgroupB}[ ( ${indexB} + processed_index_b ) % ${u32S(sharedMemorySize)} ]`)) : null,
        lessThanOrEqual: lessThanOrEqual ? ((indexA, indexB) => lessThanOrEqual(wgsl `${workgroupA}[ ( ${indexA} + processed_index_a ) % ${u32S(sharedMemorySize)} ]`, wgsl `${workgroupB}[ ( ${indexB} + processed_index_b ) % ${u32S(sharedMemorySize)} ]`)) : null
    })}
        ...
        wgsl `${workgroupA}[ ( ${indexA} + processed_index_a + thread_relative_start_a ) % ${u32S(sharedMemorySize)} ]`, wgsl `${workgroupB}[ ( ${indexB} + processed_index_b + thread_relative_start_b ) % ${u32S(sharedMemorySize)} ]`),
        setFromA: (indexOutput, indexA) => storeOutput(wgsl `( ${indexOutput} + thread_start_output )`, wgsl `${workgroupA}[ ( ${indexA} + processed_index_a + thread_relative_start_a ) % ${u32S(sharedMemorySize)} ]`),
        setFromB: (indexOutput, indexB) => storeOutput(wgsl `( ${indexOutput} + thread_start_output )`, wgsl `${workgroupB}[ ( ${indexB} + processed_index_b + thread_relative_start_b ) % ${u32S(sharedMemorySize)} ]`)
    })}
