...
export class ConsoleLogger {
    ...
    static analyze(arrayBuffer) {
        ...
        while (index < offsetLength) {
            ...
            // }
            assert && assert(info);
            const workgroupIndex = new Vector3(data[index++], data[index++], data[index++]);
            const localIndex = new Vector3(data[index++], data[index++], data[index++]);
...
ConsoleLogger.nextGlobalId = 1;
ConsoleLogger.identifierMap = new Map();
export class ConsoleLoggedEntry {
    constructor(info, data, additionalIndex, otherEntries) {
        this.info = info;
        this.data = data;
...
export class ConsoleLoggedThread {
    constructor(workgroupIndex, localIndex) {
        this.workgroupIndex = workgroupIndex;
        this.localIndex = localIndex;
        this.entries = [];
    }
    add(info, data, additionalIndex) {
...
export class ConsoleLoggedLine {
    constructor(info, additionalIndex, dataArray, 
    ...
    static toLogNull(line) {
        return null;
    }
    static toLogRaw(line) {
        return line.dataArray;
    }
    static toLogExisting(line) {
        return line.dataArray.filter(data => data !== null);
    }
    static toLogExistingFlat(line) {
...
export class ConsoleLoggedShader {
    ...
    add(info, workgroupIndex, localIndex, data, additionalIndex) {
    ...
    finalize() {
        ...
        sortedIds.forEach(id => {
            ...
            this.logLines.push(new ConsoleLoggedLine(firstEntry.info, firstEntry.additionalIndex, entries.map(entry => entry ? entry.data : null), this.threads));
        });
    }
    sortThreads() {
