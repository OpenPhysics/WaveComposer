...
export class PolygonalBoolean {
    static union(...paths) {
        return PolygonalBoolean.cag(paths, face => face.getIncludedRenderPaths().size > 0, (face, faceData, bounds) => faceData ? face.polygons : [], (faceData1, faceData2) => faceData1 === faceData2, null).flat();
    }
    static intersection(...paths) {
        return PolygonalBoolean.cag(paths, face => face.getIncludedRenderPaths().size === paths.length, (face, faceData, bounds) => faceData ? face.polygons : [], (faceData1, faceData2) => faceData1 === faceData2, null).flat();
    }
    static difference(pathA, pathB) {
        return PolygonalBoolean.cag([pathA, pathB], face => {
            const set = face.getIncludedRenderPaths();
            return set.has(pathA) && !set.has(pathB);
        }, (face, faceData, bounds) => faceData ? face.polygons : [], (faceData1, faceData2) => faceData1 === faceData2, null).flat();
    }
    static getOverlaps(pathA, pathB) {
        const taggedPolygonsList = PolygonalBoolean.cag([pathA, pathB], (face) => {
            const set = face.getIncludedRenderPaths();
        ...
        }, (face, faceData, bounds) => ({ tag: faceData, polygons: face.polygons }), (faceData1, faceData2) => faceData1 === faceData2, null);
        const result = {
            intersection: [],
            aOnly: [],
            bOnly: []
    ...
    static cag(paths, getFaceData, createOutputFace, 
    // null is for the unbounded face
    isFaceDataCompatible, log) {
        ...
        const translation = new Vector2(-0.5 * scale * (bounds.minX + bounds.maxX), -0.5 * scale * (bounds.minY + bounds.maxY));
        ...
        return RationalFace.traceCombineFaces(faces, fromIntegerMatrix, getFaceData, createOutputFace, isFaceDataCompatible, PolygonalFace.getScratchAccumulator());
    }
}
