...
class AccumulatingFace {
    constructor() {
        ...
        this.clippableFace = null;
    }
}
export class FaceConversion {
    static toSimpleRenderableFaces(faces, fromIntegerMatrix, accumulator) {
        ...
        for (let i = 0; i < faces.length; i++) {
            ...
            if (clippableFace) {
                renderableFaces.push(new RenderableFace(clippableFace, face.renderProgram, 
    ...
    static toFullyCombinedRenderableFaces(faces, fromIntegerMatrix, accumulator) {
        const faceEquivalenceClasses = [];
        for (let i = 0; i < faces.length; i++) {
            const face = faces[i];
            let found = false;
            for (let j = 0; j < faceEquivalenceClasses.length; j++) {
                const faceEquivalenceClass = faceEquivalenceClasses[j];
                const representative = faceEquivalenceClass.values().next().value;
    ...
    static toSimplifyingCombinedRenderableFaces(faces, fromIntegerMatrix, accumulator) {
        const accumulatedFaces = [];
        ...
        return accumulatedFaces.filter(accumulatedFace => !!accumulatedFace.clippableFace).map(accumulatedFace => {
            ...
            return new RenderableFace(accumulatedFace.clippableFace, accumulatedFace.renderProgram, accumulatedFace.bounds);
        });
    }
    /**
     ...
     */
    static toTracedRenderableFaces(faces, fromIntegerMatrix, accumulator) {
        return RationalFace.traceCombineFaces(faces, fromIntegerMatrix, (face) => face.renderProgram, (face, renderProgram, bounds) => new RenderableFace(face, renderProgram, bounds), (programA, programB) => {
