...
export class RationalFace {
    constructor(boundary) {
        ...
        this.holes = [];
        this.windingMapMap = new Map();
        this.windingMap = null;
        this.renderProgram = null;
        this.processed = false;
    }
    toAccumulator(accumulator, matrix = Matrix3.IDENTITY) {
        for (let i = -1; i < this.holes.length; i++) {
            ...
            for (let j = 0; j < boundary.edges.length; j++) {
                ...
                accumulator.addEdge(p0.x, p0.y, p1.x, p1.y, p0, accumulator.usesEndPoint ? p1 : null);
            }
    ...
    static traceBoundaries(filteredRationalHalfEdges, innerBoundaries, outerBoundaries, faces) {
        const zeroAreaEdges = [];
    ...
    static computeFaceHolesWithOrderedWindingNumbers(outerBoundaries, faces) {
        const exteriorBoundaries = [];
        ...
        for (let i = 0; i < outerBoundaries.length; i++) {
            ...
            for (let j = 0; j < faces.length; j++) {
                ...
                if (outerBounds.minX > innerBounds.minX &&
    ...
    static computeFaceHoles(integerBounds, outerBoundaries, faces, faceHoleLog = null) {
        let exteriorBoundary = null;
        for (let i = 0; i < outerBoundaries.length; i++) {
            const outerBoundary = outerBoundaries[i];
            const outerBounds = outerBoundary.bounds;
            const logEntry = faceHoleLog ? {
    ...
    static createUnboundedFace(...exteriorBoundaries) {
     ...
     */
    static computeWindingMaps(filteredRationalHalfEdges, unboundedFace) {
        ...
        const pendingFaces = [unboundedFace];
     ...
     */
    static traceCombineFaces(faces, fromIntegerMatrix, getFaceData, createOutputFace, 
    // null is for the unbounded face
    isFaceDataCompatible, accumulator) {
        ...
        for (let i = 0; i < faces.length; i++) {
            ...
            if (!startingFace.processed) {
                ...
                const compatibleFaces = new Set();
                const incompatibleFaces = new Set();
                compatibleFaces.add(startingFace);
                // NOTE: side effects!
                const isFaceCompatible = (candidateFace) => {
                ...
                if (clippableFace) {
                    outputFaces.push(createOutputFace(clippableFace, faceData, bounds));
                }
            }
        }
        return outputFaces;
