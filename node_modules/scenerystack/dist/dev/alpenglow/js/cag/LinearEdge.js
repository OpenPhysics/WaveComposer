...
export class LinearEdge {
    ...
    constructor(startPoint, endPoint, containsFakeCorner = false // TODO: propagate fake corners
    ) {
        this.startPoint = startPoint;
        this.endPoint = endPoint;
        this.containsFakeCorner = containsFakeCorner;
    ...
    static fromPolygon(polygon) {
        ...
        for (let i = 0; i < polygon.length; i++) {
            edges.push(new LinearEdge(polygon[i], polygon[(i + 1) % polygon.length]));
        }
        return edges;
    }
     ...
     */
    static toPolygons(edges, epsilon = 1e-8) {
        const filteredEdges = LinearEdge.withOverlappingRemoved(edges);
        const polygons = [];
        const remainingEdges = new Set(filteredEdges);
        while (remainingEdges.size > 0) {
            const edge = remainingEdges.values().next().value;
            ...
            do {
                ...
                if (edge.startPoint.equalsEpsilon(currentEdge.endPoint, epsilon)) {
                    ...
                    currentEdge = [...remainingEdges].find(candidateEdge => {
     ...
     */
    static withOppositesRemoved(edges, epsilon = 1e-8) {
        const outputEdges = [];
     ...
     * NOTE: This is a low-performance method, mainly intended for debugging display.
     */
    static withOverlappingRemoved(edges, epsilon = 1e-8) {
                ...
                if (arePointsCollinear(edge.startPoint, edge.endPoint, outputEdge.startPoint, epsilon) &&
                    ...
                    if (overlaps.length > 0) {
                        ...
                        if (assertSlow) {
                            ...
                            assertSlow((newEdgeStart.equalsEpsilon(newOutputEdgeStart, epsilon) && newEdgeEnd.equalsEpsilon(newOutputEdgeEnd, epsilon)) ||
     ...
     * NOTE: This is a low-performance method, mainly intended for debugging display.
     */
    static polygonsToShape(polygons) {
    ...
    static evaluateLineIntegralShoelaceArea(p0x, p0y, p1x, p1y) {
        return 0.5 * (p1x + p0x) * (p1y - p0y);
    }
    // Without the subexpression cancelling
    static evaluateLineIntegralArea(p0x, p0y, p1x, p1y) {
     ...
     * polygon.
     */
    static evaluateLineIntegralPartialCentroid(p0x, p0y, p1x, p1y) {
        const base = (p0x * (2 * p0y + p1y) + p1x * (p0y + 2 * p1y)) / 6;
        return new Vector2((p0x - p1x) * base, (p1y - p0y) * base);
    }
    static evaluateLineIntegralZero(p0x, p0y, p1x, p1y) {
        return (p0x - 0.1396) * (p0y + 1.422) - (p1x - 0.1396) * (p1y + 1.422);
    }
    static evaluateLineIntegralDistance(p0x, p0y, p1x, p1y) {
        const dx = p1x - p0x;
        const dy = p1y - p0y;
        ...
        return a / (6 * qd * qd * qd) * (qd * (q0 * (p0x * p0x - p0x * p1x - p0y * dy) + q1 * (kx + p1y * dy)) +
            a * a * (Math.log(logger)));
    }
    /**
     ...
     */
    getLineIntegralArea() {
        return LinearEdge.evaluateLineIntegralShoelaceArea(this.startPoint.x, this.startPoint.y, this.endPoint.x, this.endPoint.y);
    }
     ...
     */
    getLineIntegralZero() {
        return LinearEdge.evaluateLineIntegralZero(this.startPoint.x, this.startPoint.y, this.endPoint.x, this.endPoint.y);
    }
    serialize() {
        return {
    ...
    static deserialize(obj) {
        return new LinearEdge(new Vector2(obj.startPoint.x, obj.startPoint.y), new Vector2(obj.endPoint.x, obj.endPoint.y), obj.containsFakeCorner);
    }
    ...
    static getPolygonCentroid(polygon) {
        ...
        return new Vector2(x / sixArea, y / sixArea);
    }
    static getEdgesArea(clippedEdges) {
        let sum = 0;
     ...
     */
    static evaluateClosestDistanceToOrigin(p0x, p0y, p1x, p1y) {
        const dx = p1x - p0x;
        const dy = p1y - p0y;
     ...
     */
    static leftComparison(p0x, p0y, p1x, p1y, x, y) {
     ...
     */
    static windingContribution(p0x, p0y, p1x, p1y, x, y) {
        if (p0y <= y) {
    ...
    static getWindingNumberEdges(edges, point) {
        let windingNumber = 0;
        for (let i = 0; i < edges.length; i++) {
            const edge = edges[i];
            windingNumber += LinearEdge.windingContribution(edge.startPoint.x, edge.startPoint.y, edge.endPoint.x, edge.endPoint.y, point.x, point.y);
        }
        return windingNumber;
    }
    static getWindingNumberPolygon(polygon, point) {
        let windingNumber = 0;
        for (let i = 0; i < polygon.length; i++) {
            const p0 = polygon[i];
            const p1 = polygon[(i + 1) % polygon.length];
            windingNumber += LinearEdge.windingContribution(p0.x, p0.y, p1.x, p1.y, point.x, point.y);
        }
        return windingNumber;
    }
    static getWindingNumberPolygons(polygons, point) {
        let windingNumber = 0;
     ...
     * add it to the range.
     */
    static addDistanceRange(startPoint, endPoint, point, range) {
