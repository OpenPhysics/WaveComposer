...
export class ParallelRasterInitialClip {
    ...
    // read
    chunks, edges, numEdges, 
    // read-write
    clippedChunks, // Our reduce will "apply" to this, writing associated data
    // write
    edgeClips, chunkReduces, debugFullChunkReduces) {
        ...
        const kernel = new ParallelKernel(async (context) => {
            ...
            if (startPrimaryCmp === endPrimaryCmp) {
                ...
                minPoint0.set(startGreater ? (isXSplit ? new Vector2(split, startCornerSecondary) : new Vector2(startCornerSecondary, split)) : minResultStartPoint);
                minPoint1.set(minResultStartPoint);
                minPoint2.set(minResultEndPoint);
                minPoint3.set(endGreater ? (isXSplit ? new Vector2(split, endCornerSecondary) : new Vector2(endCornerSecondary, split)) : minResultEndPoint);
                maxPoint0.set(startLess ? (isXSplit ? new Vector2(split, startCornerSecondary) : new Vector2(startCornerSecondary, split)) : maxResultStartPoint);
                maxPoint1.set(maxResultStartPoint);
                maxPoint2.set(maxResultEndPoint);
                maxPoint3.set(endLess ? (isXSplit ? new Vector2(split, endCornerSecondary) : new Vector2(endCornerSecondary, split)) : maxResultEndPoint);
            }
            ...
            if (exists) {
                ...
                value = new RasterChunkReducePair(isXSplit ? new RasterChunkReduceData(minClippedChunkIndex, minArea, edge.isFirstEdge, edge.isLastEdge, minBoundsMinX, minBoundsMinY, minBoundsMaxX, minBoundsMaxY, 0, 0, minCount, 0) : new RasterChunkReduceData(minClippedChunkIndex, minArea, edge.isFirstEdge, edge.isLastEdge, minBoundsMinX, minBoundsMinY, minBoundsMaxX, minBoundsMaxY, 0, 0, 0, minCount), isXSplit ? new RasterChunkReduceData(maxClippedChunkIndex, maxArea, edge.isFirstEdge, edge.isLastEdge, maxBoundsMinX, maxBoundsMinY, maxBoundsMaxX, maxBoundsMaxY, maxCount, 0, 0, 0) : new RasterChunkReduceData(maxClippedChunkIndex, maxArea, edge.isFirstEdge, edge.isLastEdge, maxBoundsMinX, maxBoundsMinY, maxBoundsMaxX, maxBoundsMaxY, 0, maxCount, 0, 0));
            ...
            if (exists && edge.chunkIndex === firstChunkIndex) {
                context.workgroupValues.atomicMaxFirstChunkIndex = Math.max(context.workgroupValues.atomicMaxFirstChunkIndex, context.localId.x);
            }
            await context.workgroupBarrier(); // for the atomic
            // Store our reduction result
            if (exists && context.localId.x === 0) {
                const lastLocalEdgeIndexInWorkgroup = Math.min(numEdges - 1 - context.workgroupId.x * workgroupSize, workgroupSize - 1);
                ...
                await chunkReduces.set(context, context.workgroupId.x, new RasterChunkReduceQuad(leftValue.min, leftValue.max, rightValue.min, rightValue.max));
            }
        }, () => ({
    ...
    static validate(workgroupSize, chunks, edges, numEdges, clippedChunks, edgeClips, chunkReduces, debugFullChunkReduces) {
        if (assert) {
            ...
            for (let i = 0; i < Math.ceil(numEdges / workgroupSize); i++) {
                ...
                for (let j = 0; j < workgroupSize; j++) {
                    ...
                    assert((minReduce.clippedChunkIndex === maxReduce.clippedChunkIndex - 1) ||
