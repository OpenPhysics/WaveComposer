...
export class CombinedRaster {
    constructor(width, height, providedOptions) {
        ...
        assert && assert(colorSpace === 'srgb' || this.imageData.colorSpace === colorSpace, 'Check with RenderColor.canvasSupportsDisplayP3() before using display-p3');
    }
    addClientPartialPixel(color, x, y) {
    ...
    addClientFullRegion(color, x, y, width, height) {
        assert && assert(color.isFinite());
        this.addOutputFullRegion(this.colorConverter.clientToOutput(color), x, y, width, height);
    }
    addOutputFullRegion(color, x, y, width, height) {
        assert && assert(color.isFinite());
        assert && assert(isFinite(x) && isFinite(y));
        assert && assert(x >= 0 && x < this.width);
        assert && assert(y >= 0 && y < this.height);
