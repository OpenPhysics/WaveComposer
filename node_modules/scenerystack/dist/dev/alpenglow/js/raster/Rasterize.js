...
import { RasterTileLog } from './RasterLog.js';
...
const terminalAreas = [];
const terminalCentroids = [];
const terminalFinalizeCounters = [];
let terminalFaceAccumulators = [];
const terminalEdgedFaceAccumulators = [];
const terminalPolygonalFaceAccumulators = [];
class RasterizationContext {
    constructor(outputRaster, 
    // EITHER evaluator will be non-null, OR constClientColor/constOutputColor will be non-null
    evaluate, constClientColor, constOutputColor, outputRasterOffset, bounds, polygonFiltering, polygonFilterWindowMultiplier, needs, log) {
        this.outputRaster = outputRaster;
        this.evaluate = evaluate;
        this.constClientColor = constClientColor;
        this.constOutputColor = constOutputColor;
        this.outputRasterOffset = outputRasterOffset;
        this.bounds = bounds;
        this.polygonFiltering = polygonFiltering;
        this.polygonFilterWindowMultiplier = polygonFilterWindowMultiplier;
        this.needs = needs;
        this.log = log;
...
export class Rasterize {
    static getRenderProgrammedFaces(renderProgram, faces) {
        const renderProgrammedFaces = [];
    ...
    static addFilterPixel(context, pixelFace, area, x, y, color) {
        ...
        assert && assert(polygonFiltering === PolygonFilterType.Bilinear || polygonFiltering === PolygonFilterType.MitchellNetravali, 'Only supports these filters currently');
    ...
    static addPartialPixel(context, pixelFace, area, centroid, x, y) {
        ...
        if (context.constClientColor) {
            ...
            context.evaluate(scratchEvaluationContext.set(pixelFace, context.needs.needsArea ? area : NaN, centroid, x, y, x + 1, y + 1), color);
        }
        if (context.polygonFiltering === PolygonFilterType.Box) {
            ...
            assert && assert(pixelFace);
            Rasterize.addFilterPixel(context, pixelFace, area, x, y, color);
        }
    }
    static addFullArea(context, minX, minY, maxX, maxY) {
        ...
        if (constClientColor) {
            ...
            if (context.polygonFiltering === PolygonFilterType.Box) {
                if (context.constOutputColor) {
                    context.outputRaster.addOutputFullRegion(context.constOutputColor, minX + context.outputRasterOffset.x, minY + context.outputRasterOffset.y, maxX - minX, maxY - minY);
                }
                else {
                    context.outputRaster.addClientFullRegion(constClientColor, minX + context.outputRasterOffset.x, minY + context.outputRasterOffset.y, maxX - minX, maxY - minY);
                }
            }
            else {
            ...
            for (let y = minY; y < maxY; y++) {
                for (let x = minX; x < maxX; x++) {
                    const color = context.evaluate(scratchEvaluationContext.set(null, pixelArea, needs.needsCentroid ? scratchFullAreaVector.setXY(x + 0.5, y + 0.5) : nanVector, x, y, x + 1, y + 1), scratchEvaluationOutputVector);
    ...
    static terminalGridRasterize(context, clippableFace, area, minX, minY, maxX, maxY) {
        ...
        clippableFace.gridClipIterate(minX, minY, maxX, maxY, 1, 1, width, height, (cellX, cellY, startX, startY, endX, endY, startPoint, endPoint) => {
            ...
            if (needsCentroid) {
                ...
                terminalCentroids[index].addXY((startX - endX) * base, (endY - startY) * base);
            }
            if (needsFace) {
        ...
        for (let iy = 0; iy < height; iy++) {
            ...
            for (let ix = 0; ix < width; ix++) {
                ...
                if (doubleArea > 2e-8) {
                    ...
                    if (area >= 1 - 1e-8) {
                        if (context.log) {
                            context.log.fullAreas.push(new Bounds2(cellMinX, cellMinY, cellMaxX, cellMaxY));
                        }
                        Rasterize.addFullArea(context, cellMinX, cellMinY, cellMaxX, cellMaxY);
                    }
                    else {
                        ...
                        if (context.log) {
                            context.log.partialAreas.push(new Bounds2(cellMinX, cellMinY, cellMaxX, cellMaxY));
                        }
                        // NOTE: NaNs are used to hopefully hard-error if things are used that are listed as not used
                        Rasterize.addPartialPixel(context, face, area, centroid, cellMinX, cellMinY);
                    }
                }
            }
        }
    }
    static binaryRasterize(context, clippableFace, area, minX, minY, maxX, maxY) {
        assert && assert(isFinite(minX) && isFinite(minY) && isFinite(maxX) && isFinite(maxY));
        assert && assert(minX < maxX && minY < maxY);
        assert && assert(context.polygonFiltering === PolygonFilterType.Box ? Number.isInteger(minX) : minX - Math.floor(minX) === 0.5);
        assert && assert(context.polygonFiltering === PolygonFilterType.Box ? Number.isInteger(minY) : minY - Math.floor(minY) === 0.5);
        ...
        if (area >= (maxX - minX) * (maxY - minY) - 1e-8) {
            if (context.log) {
                context.log.fullAreas.push(new Bounds2(minX, minY, maxX, maxY));
            }
            Rasterize.addFullArea(context, minX, minY, maxX, maxY);
        }
        else if (xDiff === 1 && yDiff === 1) {
            if (context.log) {
                context.log.partialAreas.push(new Bounds2(minX, minY, maxX, maxY));
            }
            // NOTE: NaNs are used to hopefully hard-error if things are used that are listed as not used
            Rasterize.addPartialPixel(context, clippableFace, area, context.needs.needsCentroid ? clippableFace.getCentroid(area) : nanVector, minX, minY);
        }
        // TODO: more experimentation for when to grid clip
        else if (!context.constClientColor && xDiff <= 8 && yDiff <= 8) {
            // else if ( xDiff <= 16 && yDiff <= 16 ) {
            Rasterize.terminalGridRasterize(context, clippableFace, area, minX, minY, maxX, maxY);
        }
        else {
            ...
            if (xDiff > yDiff) {
                ...
                if (minArea > 1e-8) {
                    Rasterize.binaryRasterize(context, minFace, minArea, minX, minY, xSplit, maxY);
                }
                if (maxArea > 1e-8) {
                    Rasterize.binaryRasterize(context, maxFace, maxArea, xSplit, minY, maxX, maxY);
                }
            }
            else {
                ...
                const maxArea = maxFace.getArea();
                if (minArea > 1e-8) {
                    Rasterize.binaryRasterize(context, minFace, minArea, minX, minY, maxX, ySplit);
                }
                if (maxArea > 1e-8) {
                    Rasterize.binaryRasterize(context, maxFace, maxArea, minX, ySplit, maxX, maxY);
                }
            }
        }
    }
     ...
     * magic to avoid this.
     */
    static windowedFilterRasterize(context, clippableFace, faceBounds) {
        ...
        for (let y = outputMinY; y < outputMaxY; y++) {
            ...
            // If our filter self is outside of the face bounds, we can skip it entirely
            ...
            for (let x = outputMinX; x < outputMaxX; x++) {
                ...
                if (area > 1e-8) {
                    if (context.constClientColor) {
                        ...
                        const splitAndProcessFace = (ux, uy, face, getContribution) => {
                            ...
                            if (area > 1e-8) {
                                ...
                                const binary = (tFace, tArea, tMinX, tMinY, tMaxX, tMaxY) => {
                                    ...
                                    if (tMaxX - tMinX > 1) {
                                        ...
                                        const color = context.evaluate(scratchEvaluationContext.set(tFace, context.needs.needsArea ? tArea : NaN, // NaNs to hopefully hard-error
                                        context.needs.needsCentroid ? tFace.getCentroid(tArea) : nanVector, tMinX, tMinY, tMaxX, tMaxY), scratchEvaluationOutputVector);
    ...
    static rasterizeAccumulate(outputRaster, renderableFaces, bounds, contributionBounds, outputRasterOffset, polygonFiltering, polygonFilterWindowMultiplier, executionMethod, log) {
        ...
        for (let i = 0; i < renderableFaces.length; i++) {
            ...
            const context = new RasterizationContext(outputRaster, evaluator, constClientColor, constOutputColor, outputRasterOffset, bounds, polygonFiltering, polygonFilterWindowMultiplier, renderProgram.getNeeds(), log);
            if (polygonFilterWindowMultiplier !== 1) {
                const clipped = renderableFace.face.getClipped(contributionBounds.minX, contributionBounds.minY, contributionBounds.maxX, contributionBounds.maxY);
                ...
                const clippableFace = renderableFace.face.getClipped(faceBounds.minX, faceBounds.minY, faceBounds.maxX, faceBounds.maxY);
                Rasterize.binaryRasterize(context, clippableFace, clippableFace.getArea(), faceBounds.minX, faceBounds.minY, faceBounds.maxX, faceBounds.maxY);
            }
        }
    }
    static markStart(name) {
        self.performance && self.performance.mark(`${name}-start`);
    }
    static markEnd(name) {
        self.performance && self.performance.mark(`${name}-end`);
        self.performance && self.performance.measure(name, `${name}-start`, `${name}-end`);
    }
    static partitionRenderableFaces(renderProgram, bounds, providedOptions) {
        // Coordinate frames:
        //
        ...
        const polygonFiltering = options.polygonFiltering;
        ...
        for (let y = contributionBounds.minY; y < contributionBounds.maxY; y += tileSize) {
            for (let x = contributionBounds.minX; x < contributionBounds.maxX; x += tileSize) {
                ...
                if (log && tileLog) {
                    log.tileLogs.push(tileLog);
                }
                // A slice of our contributionBounds
                const tileBounds = new Bounds2(x, y, Math.min(x + tileSize, contributionBounds.maxX), Math.min(y + tileSize, contributionBounds.maxY));
                // -( scale * ( tileBounds.minX + filterGridOffset.x ) + translation.x ) = scale * ( tileBounds.maxX + filterGridOffset.x ) + translation.x
                const translation = new Vector2(-0.5 * scale * (tileBounds.minX + tileBounds.maxX), -0.5 * scale * (tileBounds.minY + tileBounds.maxY));
                if (tileLog) {
                    tileLog.translation = translation;
                }
                const toIntegerMatrix = Matrix3.affine(scale, 0, translation.x, 0, scale, translation.y);
                if (tileLog) {
                    tileLog.toIntegerMatrix = toIntegerMatrix;
                }
                const fromIntegerMatrix = toIntegerMatrix.inverted();
                if (tileLog) {
                    tileLog.fromIntegerMatrix = fromIntegerMatrix;
                }
                ...
                if (tileLog) {
                    tileLog.integerEdges = integerEdges;
                }
                ...
                if (tileLog) {
                    tileLog.filteredRationalHalfEdges = filteredRationalHalfEdges;
                }
                ...
                if (tileLog) {
                    tileLog.refilteredRationalHalfEdges = filteredRationalHalfEdges;
                }
                log && Rasterize.markStart('face-holes');
                const exteriorBoundaries = RationalFace.computeFaceHolesWithOrderedWindingNumbers(outerBoundaries, faces);
                ...
                if (tileLog) {
                    tileLog.unboundedFace = unboundedFace;
                }
                ...
                if (tileLog) {
                    tileLog.renderedFaces = renderedFaces;
                }
                ...
                if (tileLog) {
                    tileLog.initialRenderableFaces = renderableFaces;
                }
                ...
                if (tileLog) {
                    tileLog.renderableFaces = renderableFaces;
                }
        ...
        if (log) {
            log.renderableFaces = combinedRenderableFaces;
        }
        return combinedRenderableFaces;
    }
    static rasterize(renderProgram, outputRaster, bounds, providedOptions) {
        const options = optionize3()({}, DEFAULT_OPTIONS, providedOptions);
        const log = options.log;
        ...
        Rasterize.rasterizeAccumulate(outputRaster, renderableFaces, bounds, contributionBounds, options.outputRasterOffset, polygonFiltering, polygonFilterWindowMultiplier, options.executionMethod, log);
    ...
    static writeImageDataToCanvas(imageData, canvas, context) {
