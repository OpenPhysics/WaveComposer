...
export default class FineCoarseSpinner extends AccessibleNumberSpinner(Node, 0) {
    constructor(numberProperty, providedOptions) {
        ...
        }), combineOptions({}, fineButtonOptions, { tandem: options.tandem.createTandem('decrementFineButton') }));
        // <<
        ...
        const incrementFineButton = new ArrowButton('right', (() => {
            numberProperty.value = numberProperty.value + options.deltaFine;
        }), combineOptions({}, fineButtonOptions, { tandem: options.tandem.createTandem('incrementFineButton') }));
        // >>
        const incrementCoarseButton = new ArrowButton('right', (() => {
            ...
            numberProperty.value = numberProperty.value + delta;
        }), combineOptions({}, coarseButtonOptions, { tandem: options.tandem.createTandem('incrementCoarseButton') }));
        // <  <<  [ value ]  >>  >
        const hBox = new HBox({
        ...
        assert && self.phet?.chipper?.queryParameters?.binder && InstanceRegistry.registerDataURL('scenery-phet', 'FineCoarseSpinner', this);
    }
    dispose() {
        this.disposeFineCoarseSpinner();
        super.dispose();
