...
export default class SpectrumSlider extends AccessibleSlider(Node, 0) {
    ...
    constructor(valueProperty, providedOptions) {
        ...
        const positionToValue = (x) => clamp(linear(0, track.width, options.minValue, options.maxValue, x), options.minValue, options.maxValue);
        const valueToPosition = (value) => clamp(linear(options.minValue, options.maxValue, 0, track.width, value), 0, track.width);
        ...
        assert && self.phet?.chipper?.queryParameters?.binder && InstanceRegistry.registerDataURL('scenery-phet', 'SpectrumSlider', this);
    }
    dispose() {
        this.disposeSpectrumSlider();
        super.dispose();
