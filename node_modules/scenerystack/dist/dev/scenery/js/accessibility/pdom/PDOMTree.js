...
export default class PDOMTree {
     ...
     */
    static childrenOrderChange(node) {
     ...
     */
    static pdomOrderChange(node, oldOrder, newOrder) {
        ...
        if (assert) {
            for (i = 0; i < removedItems.length; i++) {
                ...
                assert(item === null || item._pdomParent === node, 'Node should have had a pdomOrder');
            }
            for (i = 0; i < addedItems.length; i++) {
                const item = addedItems[i];
                assert(item === null || item._pdomParent === null, 'Node is already specified in a pdomOrder');
            }
        }
     ...
     */
    static rebuildInstanceTree(rootInstance) {
     ...
     */
    static addTree(parent, child, pdomTrails) {
     ...
     * @param [pdomTrails] - Will be computed if needed
     */
    static removeTree(parent, child, pdomTrails) {
     ...
     */
    static reorder(node, pdomTrails) {
     ...
     */
    static createTree(trail, display, parentInstance) {
        ...
        assert && assert(!node._lockedPDOMInstanceCreation, 'You are recursively creating another PDOMInstance for the same Node! Probably because a child instance is triggering a createTree on a parent.');
     ...
     */
    static recursivePDOMTrailSearch(trailResults, trail) {
     ...
     */
    static auditNodeForPDOMCycles(node) {
        if (assert) {
            const trail = new Trail(node);
            (function recursiveSearch() {
                const root = trail.rootNode();
                assert(trail.length <= 1 || root !== node, `${'Accessible PDOM graph cycle detected. The combined scene-graph DAG with pdomOrder defining additional ' +
                    'parent-child relationships should still be a DAG. Cycle detected with the trail: '}${trail.toString()} path: ${trail.toPathString()}`);
