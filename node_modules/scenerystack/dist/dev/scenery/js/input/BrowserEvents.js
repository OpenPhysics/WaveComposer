...
class BrowserEvents {
    ...
    static addDisplay(display, attachToWindow, passiveEvents) {
        assert && assert(!_.includes(BrowserEvents.attachedDisplays, display), 'A display cannot be concurrently attached to events more than one time');
        // Always first please
    ...
    static removeDisplay(display, attachToWindow, passiveEvents) {
        assert && assert(_.includes(BrowserEvents.attachedDisplays, display), 'This display was not already attached to listen for self events');
    ...
    static addOrRemoveListeners(element, addOrRemove, passiveEvents) {
        ...
        assert && assert(!forWindow || (BrowserEvents.listenersAttachedToWindow > 0) === !addOrRemove, 'Do not add listeners to the self when already attached, or remove listeners when none are attached');
        ...
        assert && assert(BrowserEvents.listenersAttachedToWindow === 0 || BrowserEvents.listenersAttachedToElement === 0, 'Listeners should not be added both with addDisplayToWindow and addDisplayToElement. Use only one.');
