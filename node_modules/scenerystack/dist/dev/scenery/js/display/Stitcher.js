        ...
        if (assertSlow) {
            ...
            assertSlow(this.reindexed || blocks.length === 0 ||
                ...
                (previousBlocks.length === blocks.length &&
                    _.every(_.zip(previousBlocks, blocks), arr => arr[0] === arr[1])), 'Did not reindex on a block change where we are left with blocks');
            ...
            if (blocks.length) {
                assertSlow(this.backbone.previousFirstDrawable !== null &&
                    this.backbone.previousLastDrawable !== null, 'If we are left with at least one block, we must be tracking at least one drawable');
                ...
                for (let i = 0; i < blocks.length - 1; i++) {
                    ...
                    assertSlow(blocks[i].pendingLastDrawable.nextDrawable === blocks[i + 1].pendingFirstDrawable &&
                        blocks[i].pendingLastDrawable === blocks[i + 1].pendingFirstDrawable.previousDrawable, 'Consecutive blocks should have boundary drawables that are also consecutive in the linked list');
                }
            }
            else {
                assertSlow(this.backbone.previousFirstDrawable === null &&
                    this.backbone.previousLastDrawable === null, 'If we are left with no blocks, it must mean we are tracking precisely zero drawables');
            }
        }
    }
    /**
