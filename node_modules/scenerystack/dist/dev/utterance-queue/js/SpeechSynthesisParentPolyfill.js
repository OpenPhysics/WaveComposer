...
const SpeechSynthesisParentPolyfill = {
    /**
     * Initialize the polyfill, setting objects related to SpeechSynthesis on the self. The implementation of
     * SpeechSynthesisPolyfill assumes that the simulation is running in a child iframe under a parent window
     * that implements SpeechSynthesis. So we grab the implementation from the parent and set it to this self.
     */
    initialize() {
        if (self.speechSynthesis || self.SpeechSynthesis || self.SpeechSynthesisUtterance) {
            throw new Error('SpeechSynthesis is supported here, the polyfill should not overwrite it');
        }
        assert && assert(self.parent, 'This polyfill requires a parent frame implementation of SpeechSynthesis.');
        // @ts-expect-error - Very hacky, this prototype solution is not getting work anymore. No need for better solution.
        self.SpeechSynthesis = self.parent.SpeechSynthesis;
        // @ts-expect-error - Very hacky, this prototype solution is not getting work anymore. No need for better solution.
        self.speechSynthesis = new self.parent.SpeechSynthesis();
        // @ts-expect-error - Very hacky, this prototype solution is not getting work anymore. No need for better solution.
        self.SpeechSynthesisUtterance = self.parent.SpeechSynthesisUtterance;
    }
};
