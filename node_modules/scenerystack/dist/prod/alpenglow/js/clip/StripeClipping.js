...
export class StripeClipping {
    ...
    static binaryStripeClipEdge(startPoint, endPoint, normal, // NOTE: does NOT need to be a unit vector
    values, fakeCornerPerpendicular, clippedEdgeCollection // Will append into this (for performance)
    ) {
        ...
        for (let j = 0; j < values.length + 1; j++) {
            ...
            let minFakeCorner = null;
            let maxFakeCorner = null;
            if (startDot < minValue || endDot < minValue) {
    ...
    // line where dot( normal, point ) - value = 0. "min" side is dot-products < value, "max" side is dot-products > value
    static binaryStripeClipPolygon(polygon, normal, // NOTE: does NOT need to be a unit vector
    values // SHOULD BE SORTED from low to high -- no duplicates (TODO verify, enforce in gradients)
    ) {
        const perpendicular = normal.perpendicular;
        const basePoints = values.map(value => normal.timesScalar(value / normal.dot(normal)));
        const perpPerp = perpendicular.dot(perpendicular);
...
//# sourceMappingURL=StripeClipping.js.map