...
export class ParallelRasterEdgeScan {
    ...
    // read
    clippedChunks, edgeClips, edgeReduces0, edgeReduces1, edgeReduces2, numEdgeClips, 
    // write
    reducibleEdges, completeEdges, chunkIndices, debugEdgeScan) {
        ...
        const kernel = new ParallelKernel(async (context) => {
            ...
            if (exists) {
                ...
                await debugEdgeScan.set(context, context.globalId.x, new RasterSplitReduceData(baseReducible + value.numReducible - initialValue.numReducible, baseComplete + value.numComplete - initialValue.numComplete));
                ...
                if (hasReducibleVertices) {
                    ...
                    if (index > 0) {
                        await reducibleEdges.set(context, baseIndex, new RasterEdge(edgeClip.clippedChunkIndex, false, false, // will get filled in later
                        edgeStarts[0], edgeEnds[0]));
                        if (index > 1) {
                            await reducibleEdges.set(context, baseIndex + 1, new RasterEdge(edgeClip.clippedChunkIndex, false, false, // will get filled in later
                            edgeStarts[1], edgeEnds[1]));
                            if (index > 2) {
                                await reducibleEdges.set(context, baseIndex + 2, new RasterEdge(edgeClip.clippedChunkIndex, false, false, // will get filled in later
                ...
                if (hasCompleteVertices) {
                    ...
                    if (index > 0) {
                        await completeEdges.set(context, baseIndex, new RasterCompleteEdge(edgeStarts[0], edgeEnds[0]));
                        if (index > 1) {
                            await completeEdges.set(context, baseIndex + 1, new RasterCompleteEdge(edgeStarts[1], edgeEnds[1]));
                            if (index > 2) {
                                await completeEdges.set(context, baseIndex + 2, new RasterCompleteEdge(edgeStarts[2], edgeEnds[2]));
                            }
                        }
                    }
                }
...
//# sourceMappingURL=ParallelRasterEdgeScan.js.map