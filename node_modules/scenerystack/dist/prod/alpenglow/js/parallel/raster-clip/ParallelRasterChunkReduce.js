    ...
    outputChunkReduces) {
        ...
        const kernel = new ParallelKernel(async (context) => {
            ...
            if (exists && clippedChunkIndex === firstClippedChunkIndex) {
                context.workgroupValues.atomicMaxFirstReduceIndex = Math.max(context.workgroupValues.atomicMaxFirstReduceIndex, context.localId.x);
            }
            await context.workgroupBarrier(); // for the atomic
            // Store our reduction result
            if (exists && context.localId.x === 0) {
                const lastLocalEdgeIndexInWorkgroup = Math.min(numReduces - 1 - context.workgroupId.x * workgroupSize, workgroupSize - 1);
                ...
                await outputChunkReduces.set(context, context.workgroupId.x, new RasterChunkReduceQuad(leftValue.leftMin, leftValue.leftMax, rightValue.rightMin, rightValue.rightMax));
            }
        }, () => ({
...
//# sourceMappingURL=ParallelRasterChunkReduce.js.map