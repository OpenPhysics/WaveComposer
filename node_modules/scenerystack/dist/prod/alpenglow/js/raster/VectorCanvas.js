...
export class VectorCanvas {
    ...
    polygonFiltering = PolygonFilterType.Box) {
        ...
        this.gridBounds = Bounds2.NOTHING;
        this.updateWidthHeight(width, height);
    }
    // Assumes sRGB? We'll do srgb-linear blending to mimic for now?
    fillColor(renderPath, color) {
        this.fillRenderProgram(renderPath, this.colorToRenderProgram(color));
    }
    fillLinearGradient(renderPath, start, end, stops, extend, accuracy = RenderLinearGradientAccuracy.SplitAccurate) {
        this.fillRenderProgram(renderPath, new RenderLinearGradient(Matrix3.IDENTITY, start, end, stops.map(stop => new RenderGradientStop(stop.ratio, this.colorToRenderProgram(stop.color))), extend, accuracy));
        this.renderableFaces = this.renderableFaces.flatMap(face => face.split());
    }
    fillRadialGradient(renderPath, transform, start, startRadius, end, endRadius, stops, extend, accuracy = RenderRadialGradientAccuracy.SplitAccurate) {
        this.fillRenderProgram(renderPath, new RenderRadialGradient(Matrix3.IDENTITY, start, startRadius, end, endRadius, stops.map(stop => new RenderGradientStop(stop.ratio, this.colorToRenderProgram(stop.color))), extend, accuracy));
        this.renderableFaces = this.renderableFaces.flatMap(face => face.split());
    }
    copy() {
    ...
    fillRenderProgram(renderPath, renderProgram) {
        const bounds = renderPath.getBounds();
        renderPath = new RenderPath(renderPath.fillRule, renderPath.subpaths.map(subpath => BoundsClipping.boundsClipPolygon(subpath, this.bounds.minX, this.bounds.minY, this.bounds.maxX, this.bounds.maxY, this.bounds.centerX, this.bounds.centerY)));
        const newRenderableFaces = [];
            ...
            if (renderableFace.bounds.intersectsBounds(bounds)) {
                ...
                if (overlaps.intersection.length) {
                    if (overlaps.aOnly.length) {
                        ...
                        newRenderableFaces.push(new RenderableFace(aOnlyFace, renderableFace.renderProgram, aOnlyFace.getBounds()));
                    }
                    const intersectionFace = new PolygonalFace(overlaps.intersection);
                    newRenderableFaces.push(new RenderableFace(intersectionFace, new RenderStack([renderableFace.renderProgram, renderProgram]).simplified(), intersectionFace.getBounds()));
                }
                else {
    ...
    combineFaces() {
        ...
        for (let i = 0; i < this.renderableFaces.length; i++) {
            ...
            if (compatibleFaces.length) {
                ...
                this.renderableFaces[i] = new RenderableFace(newFace, renderProgram, newFace.getBounds());
    ...
    updateWidthHeight(width, height) {
        ...
        this.renderableFaces.push(new RenderableFace(PolygonalFace.fromBounds(this.bounds), RenderColor.TRANSPARENT, this.bounds));
        // TODO: splitting of radial/linear gradients!!!
    }
    getImageData(options) {
        const raster = new CombinedRaster(this.width, this.height, combineOptions({
            colorSpace: this.colorSpace
        }, options));
        Rasterize.rasterizeAccumulate(raster, this.renderableFaces, this.bounds, this.gridBounds, Vector2.ZERO, this.polygonFiltering, 1, 'evaluation', null);
...
//# sourceMappingURL=VectorCanvas.js.map