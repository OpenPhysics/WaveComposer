...
class ItemSegmentTree extends SegmentTree {
    getMinX(item, epsilon) {
        return item.bounds.left - epsilon;
    }
    getMaxX(item, epsilon) {
        return item.bounds.right + epsilon;
    }
}
export class BoundsIntersectionFilter {
    static quadraticIntersect(maximumBounds, items, callback) {
    ...
    static sweepLineIntersect(maximumBounds, items, callback) {
    ...
    static filterIntersect(maximumBounds, items, callback) {
        BoundsIntersectionFilter.recurse(Orientation.HORIZONTAL, maximumBounds, items, [], false, new OrientationPair(false, false), new OrientationPair(false, false), new OrientationPair(false, false), 0, callback);
    }
    static intersect(internalItems, externalItems, external, callback) {
    ...
    static recurse(orientation, bounds, internalItems, externalItems, otherComplete, external, minSplitLast, clipped, depth, callback) {
        ...
        const bailOrientation = () => {
            if (otherComplete) {
                ...
                BoundsIntersectionFilter.recurse(orientation.opposite, bounds, internalItems, externalItems, true, external, minSplitLast, clipped, depth + 1, callback);
            }
        };
        let split = 0;
        ...
        BoundsIntersectionFilter.recurse(otherComplete ? orientation : orientation.opposite, minBounds, minActiveItems, [
            ...minInactiveItems,
            ...bothInactiveItems,
            ...(isOrientationExternal ? [] : bothActiveItems)
        ], otherComplete, external, minSplit, newClipped, depth + 1, callback);
        // "maximum" case, internal for "max", moving "both" to external
        BoundsIntersectionFilter.recurse(otherComplete ? orientation : orientation.opposite, maxBounds, maxActiveItems, [
            ...maxInactiveItems,
            ...bothInactiveItems,
            ...(isOrientationExternal ? [] : bothActiveItems)
        ], otherComplete, external, maxSplit, newClipped, depth + 1, callback);
        if (otherComplete) {
            ...
            BoundsIntersectionFilter.recurse(orientation.opposite, bounds, bothActiveItems, bothInactiveItems, true, external, minSplitLast, clipped, // we did NOT change clips!
            depth + 1, callback);
        }
        if (!isOrientationExternal) {
            // min external "both" case
            BoundsIntersectionFilter.recurse(orientation, minBounds, bothActiveItems, minInactiveItems, otherComplete, newExternal, minSplit, newClipped, depth + 1, callback);
            // max external "both" case
            BoundsIntersectionFilter.recurse(orientation, maxBounds, bothActiveItems, maxInactiveItems, otherComplete, newExternal, maxSplit, newClipped, depth + 1, callback);
        }
    }
}
//# sourceMappingURL=BoundsIntersectionFilter.js.map