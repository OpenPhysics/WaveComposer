...
import { GridClipping } from '../clip/GridClipping.js';
...
export const serializeClippableFace = (clippableFace) => {
    ;
    return {
...
export const deserializeClippableFace = (serialized) => {
 ...
 * @author Jonathan Olson <jonathan.olson@colorado.edu>
 */
export class EdgedFace {
    constructor(edges, skipValidation = false) {
        this.edges = edges;
     ...
     */
    toEdgedClippedFace(minX, minY, maxX, maxY) {
     ...
     */
    toEdgedClippedFaceWithoutCheck(minX, minY, maxX, maxY) {
     ...
     */
    getDistanceRangeToInside(point) {
     ...
     */
    getAverageDistance(point, area) {
        let sum = 0;
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
            const p0 = edge.startPoint;
            const p1 = edge.endPoint;
            sum += LinearEdge.evaluateLineIntegralDistance(p0.x - point.x, p0.y - point.y, p1.x - point.x, p1.y - point.y);
        }
        return sum / area;
    }
    /**
     ...
     */
    getAverageDistanceTransformedToOrigin(transform, area) {
        let sum = 0;
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
     ...
     */
    getClipped(minX, minY, maxX, maxY) {
        const edges = [];
        const centerX = (minX + maxX) / 2;
        const centerY = (minY + maxY) / 2;
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
            BoundsClipping.boundsClipEdge(edge.startPoint, edge.endPoint, minX, minY, maxX, maxY, centerX, centerY, edges);
        }
     ...
     */
    getBinaryLineClip(normal, value, fakeCornerPerpendicular) {
        const minEdges = [];
        const maxEdges = [];
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
     ...
     */
    getStripeLineClip(normal, values, fakeCornerPerpendicular) {
        if (values.length === 0) {
            return [this];
        }
        const edgesCollection = _.range(values.length + 1).map(() => []);
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
     ...
     */
    getBinaryCircularClip(center, radius, maxAngleSplit) {
        const insideEdges = [];
        const outsideEdges = [];
     ...
     */
    gridClipIterate(minX, minY, maxX, maxY, stepX, stepY, stepWidth, stepHeight, callback, polygonCompleteCallback) {
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
            GridClipping.gridClipIterate(edge.startPoint, edge.endPoint, minX, minY, maxX, maxY, stepX, stepY, stepWidth, stepHeight, callback);
        }
     ...
     */
    getBilinearFiltered(pointX, pointY, minX, minY) {
     ...
     */
    getMitchellNetravaliFiltered(pointX, pointY, minX, minY) {
    ...
    getRounded(epsilon) {
        ...
        for (let i = 0; i < this.edges.length; i++) {
            const edge = this.edges[i];
            const startPoint = new Vector2(roundSymmetric(edge.startPoint.x / epsilon) * epsilon, roundSymmetric(edge.startPoint.y / epsilon) * epsilon);
            const endPoint = new Vector2(roundSymmetric(edge.endPoint.x / epsilon) * epsilon, roundSymmetric(edge.endPoint.y / epsilon) * epsilon);
     ...
     */
    withReversedEdges() {
        return new EdgedFace(this.edges.map(edge => new LinearEdge(edge.endPoint, edge.startPoint)).reverse());
    }
    /**
    ...
    static fromBoundsValues(minX, minY, maxX, maxY) {
...
export class EdgedFaceAccumulator {
    constructor() {
        this.edges = [];
        this.usesEndPoint = true;
    }
    addEdge(startX, startY, endX, endY, startPoint, endPoint) {
        ;
        this.edges.push(new LinearEdge(startPoint || new Vector2(startX, startY), endPoint || new Vector2(endX, endY)));
    }
    markNewPolygon() {
        // no-op, since we're storing unsorted edges!
    }
    setAccumulationBounds(minX, minY, maxX, maxY) {
...
export class EdgedClippedFace {
    ...
    static fromEdges(edges, minX, minY, maxX, maxY) {
    ...
    static fromEdgesWithoutCheck(edges, minX, minY, maxX, maxY) {
        return new EdgedClippedFace(edges, minX, minY, maxX, maxY, 0, 0, 0, 0);
    }
    static implicitEdge(startPoint, endPoint, count) {
        ;
        return new LinearEdge(count > 0 ? startPoint : endPoint, count > 0 ? endPoint : startPoint);
    }
    forEachImplicitEdge(callback) {
    ...
    getClipped(minX, minY, maxX, maxY) {
        ...
        this.forEachImplicitEdge((startPoint, endPoint) => {
            BoundsClipping.boundsClipEdge(startPoint, endPoint, minX, minY, maxX, maxY, centerX, centerY, edges);
        });
    ...
    gridClipIterate(minX, minY, maxX, maxY, stepX, stepY, stepWidth, stepHeight, callback, polygonCompleteCallback) {
        ...
        this.forEachImplicitEdge((startPoint, endPoint) => {
            GridClipping.gridClipIterate(startPoint, endPoint, minX, minY, maxX, maxY, stepX, stepY, stepWidth, stepHeight, callback);
        });
        if (this.edges.length) {
            polygonCompleteCallback();
        }
    ...
    containsPoint(point) {
        ...
        this.forEachImplicitEdge((startPoint, endPoint) => {
            windingNumber += LinearEdge.windingContribution(startPoint.x, startPoint.y, endPoint.x, endPoint.y, point.x, point.y);
        });
    ...
    getRounded(epsilon) {
        ...
        this.forEachImplicitEdge((startPoint, endPoint) => {
            const roundedStartPoint = new Vector2(roundSymmetric(startPoint.x / epsilon) * epsilon, roundSymmetric(startPoint.y / epsilon) * epsilon);
            const roundedEndPoint = new Vector2(roundSymmetric(endPoint.x / epsilon) * epsilon, roundSymmetric(endPoint.y / epsilon) * epsilon);
            if (!roundedStartPoint.equals(roundedEndPoint)) {
                edges.push(new LinearEdge(roundedStartPoint, roundedEndPoint));
            }
        });
        return EdgedClippedFace.fromEdgesWithoutCheck(edges, roundSymmetric(this.minX / epsilon) * epsilon, roundSymmetric(this.minY / epsilon) * epsilon, roundSymmetric(this.maxX / epsilon) * epsilon, roundSymmetric(this.maxY / epsilon) * epsilon);
    }
    /**
     * Returns a version of the face with the orientation of all of the edges swapped.
     */
    withReversedEdges() {
        return new EdgedClippedFace(this.edges.map(edge => new LinearEdge(edge.endPoint, edge.startPoint)).reverse(), this.minX, this.minY, this.maxX, this.maxY, -this.maxXCount, -this.maxYCount, -this.minXCount, -this.minYCount);
    }
    /**
     * Calls the callback with points for each edge in the face.
     */
    ...
    static deserialize(serialized) {
        return new EdgedClippedFace(serialized.edges.map(edge => LinearEdge.deserialize(edge)), serialized.minX, serialized.minY, serialized.maxX, serialized.maxY, serialized.minXCount, serialized.minYCount, serialized.maxXCount, serialized.maxYCount);
    }
    static fromBounds(bounds) {
        return new EdgedClippedFace([], bounds.minX, bounds.minY, bounds.maxX, bounds.maxY, -1, 1, 1, -1);
    }
    static fromBoundsValues(minX, minY, maxX, maxY) {
        return new EdgedClippedFace([], minX, minY, maxX, maxY, -1, 1, 1, -1);
    }
}
export class EdgedClippedFaceAccumulator {
    constructor() {
        this.edges = [];
        this.minX = 0;
        this.minY = 0;
    ...
    finalizeEnsureFace(minX, minY, maxX, maxY) {
...
export class PolygonalFace {
     ...
     */
    getStripeLineClip(normal, values, fakeCornerPerpendicular) {
        const polygonsCollection = _.range(values.length + 1).map(() => []);
        for (let i = 0; i < this.polygons.length; i++) {
            const polygon = this.polygons[i];
    ...
    getRounded(epsilon) {
        return new PolygonalFace(this.polygons.map(polygon => polygon.map(vertex => {
            return new Vector2(roundSymmetric(vertex.x / epsilon) * epsilon, roundSymmetric(vertex.y / epsilon) * epsilon);
        })));
    }
    /**
     * Returns a version of the face with the orientation of all of the edges swapped.
     */
    withReversedEdges() {
        return new PolygonalFace(this.polygons.map(polygon => polygon.slice().reverse()));
    }
    /**
     * Calls the callback with points for each edge in the face.
     */
...
export class PolygonalFaceAccumulator {
    constructor() {
        this.polygons = [];
        ...
        this.simplifier = new ClipSimplifier();
        this.usesEndPoint = false;
    }
    addEdge(startX, startY, endX, endY, startPoint, endPoint) {
        ;
...
export class CircularClipping {
     ...
     */
    static binaryCircularClipEdges(edges, center, radius, maxAngleSplit, inside, outside) {
        ...
        const processExternal = (edge, startInside, endInside) => {
            outside.push(edge);
            const localStart = edge.startPoint.minus(center);
            const localEnd = edge.endPoint.minus(center);
            ...
            if (!localStart.equalsEpsilon(localEnd, 1e-8)) {
                ...
                insideCircularEdges.push(new CircularEdgeWithPoints(startInside ? edge.startPoint : null, endInside ? edge.endPoint : null, startAngle, endAngle, !isClockwise));
            }
            else {
     ...
     */
    static binaryCircularClipPolygon(polygons, center, radius, maxAngleSplit, inside, outside) {
        ...
        const insideCandidatePolygons = [];
        const outsideCandidatePolygons = [];
        ...
        const outsideCandidateForwardEdges = [];
        const outsideCandidateReversedEdges = [];
        ...
        const outsideStartOutsideCandidateForwardEdges = [];
        const outsideStartOutsideCandidateReversedEdges = [];
        // We'll also need to store "critical" angles for the future polygonalization of the circles. If we were outputting
        // true circular edges, we could just include `insideCircularEdges`, however we want to convert it to line segments
        // so that future stages don't have to deal with this.
        // We'll need the angles so that those points on the circle will be exact (for ALL of the circular edges).
        ...
        // Process a fully-inside-the-circle part of an edge
        const processInternal = (start, end) => {
        ...
        // Process a fully-outside-the-circle part of an edge
        const processExternal = (start, end) => {
        ...
        const addEdgeTo = (edge, simplifier) => {
        ...
        const addPolygonTo = (edges, polygons) => {
     ...
     * TODO: test this!
     */
    static binaryCircularTracingClipIterate(
    // TODO: can we do this from a stream of data instead?
    polygons, center, radius, maxAngleSplit, callback, polygonCompleteCallback) {
        const radiusSquared = radius * radius;
        // If we inscribed a circle inside a regular polygon split at angle `maxAngleSplit`, we'd have this radius.
        // Because we're turning our circular arcs into line segments at the end, we need to make sure that content inside
        // the circle doesn't go OUTSIDE the "inner" polygon (in that sliver between the circle and regular polygon).
        ...
        const addPolygonTo = (edges, isInside) => {
            for (let j = 0; j < edges.length; j++) {
...
class CircularEdge {
    constructor(startAngle, endAngle, counterClockwise) {
...
class CircularEdgeWithPoints {
    constructor(startPoint, endPoint, startAngle, endAngle, counterClockwise) {
...
export class BinaryClipping {
    // Returns if all done
    static binaryInitialPush(startPoint, endPoint, startCmp, endCmp, minLinearEdges, maxLinearEdges) {
    ...
    static binaryPushClipEdges(startPoint, endPoint, startCmp, endCmp, fakeCorner, intersection, minLinearEdges, maxLinearEdges) {
    ...
    static binaryXClipEdge(startPoint, endPoint, x, fakeCornerY, minLinearEdges, // Will append into this (for performance)
    maxLinearEdges // Will append into this (for performance)
    ) {
        const startCmp = Math.sign(startPoint.x - x);
        const endCmp = Math.sign(endPoint.x - x);
        const handled = this.binaryInitialPush(startPoint, endPoint, startCmp, endCmp, minLinearEdges, maxLinearEdges);
        if (handled) {
            return;
        }
        ...
        BinaryClipping.binaryPushClipEdges(startPoint, endPoint, startCmp, endCmp, fakeCorner, intersection, minLinearEdges, maxLinearEdges);
    }
    static binaryYClipEdge(startPoint, endPoint, y, fakeCornerX, minLinearEdges, // Will append into this (for performance)
    maxLinearEdges // Will append into this (for performance)
    ) {
    ...
    static binaryLineClipEdge(startPoint, endPoint, normal, // NOTE: does NOT need to be a unit vector
    value, fakeCornerPerpendicular, minLinearEdges, // Will append into this (for performance)
    maxLinearEdges // Will append into this (for performance)
    ) {
    ...
    static binaryXClipPolygon(polygon, x, minPolygon, // Will append into this (for performance)
    maxPolygon // Will append into this (for performance)
    ) {
    ...
    static binaryYClipPolygon(polygon, y, minPolygon, // Will append into this (for performance)
    maxPolygon // Will append into this (for performance)
    ) {
        for (let i = 0; i < polygon.length; i++) {
            const startPoint = polygon[i];
    ...
    // line where dot( normal, point ) - value = 0. "min" side is dot-products < value, "max" side is dot-products > value
    static binaryLineClipPolygon(polygon, normal, // NOTE: does NOT need to be a unit vector
    value, minPolygon, // Will append into this (for performance)
    maxPolygon // Will append into this (for performance)
    ) {
        const perpendicular = normal.perpendicular;
        const basePoint = normal.timesScalar(value / normal.dot(normal));
    ...
    static binaryXClipEdgedClipped(face, x) {
        const minEdges = [];
        const maxEdges = [];
        let minCount = 0;
        let maxCount = 0;
        ...
        const minFace = new EdgedClippedFace(minEdges, face.minX, face.minY, x, face.maxY, face.minXCount, face.minYCount, face.maxXCount + minCount, face.maxYCount);
        const maxFace = new EdgedClippedFace(maxEdges, x, face.minY, face.maxX, face.maxY, face.minXCount + maxCount, face.minYCount, face.maxXCount, face.maxYCount);
        false;
        return {
            minFace: minFace,
            maxFace: maxFace
    ...
    static binaryYClipEdgedClipped(face, y) {
        const minEdges = [];
        const maxEdges = [];
        let minCount = 0;
        let maxCount = 0;
        ...
        const minFace = new EdgedClippedFace(minEdges, face.minX, face.minY, face.maxX, y, face.minXCount, face.minYCount, face.maxXCount, face.maxYCount + minCount);
        const maxFace = new EdgedClippedFace(maxEdges, face.minX, y, face.maxX, face.maxY, face.minXCount, face.minYCount + maxCount, face.maxXCount, face.maxYCount);
        false;
        return {
            minFace: minFace,
            maxFace: maxFace
...
//# sourceMappingURL=ClippableFace.js.map