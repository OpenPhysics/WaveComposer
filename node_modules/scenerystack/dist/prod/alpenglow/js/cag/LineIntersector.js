...
export class LineIntersector {
    static processIntegerEdgeIntersection(edgeA, edgeB, log) {
        ...
        const intersectionPoints = IntersectionPoint.intersectLineSegments(new BigIntVector2(BigInt(edgeA.x0), BigInt(edgeA.y0)), new BigIntVector2(BigInt(edgeA.x1), BigInt(edgeA.y1)), new BigIntVector2(BigInt(edgeB.x0), BigInt(edgeB.y0)), new BigIntVector2(BigInt(edgeB.x1), BigInt(edgeB.y1)));
        if (log) {
    ...
    static edgeIntersectionBoundsTree(integerEdges, log) {
        BoundsTreeNode.fromIntegerEdges(integerEdges).selfIntersect(log);
    }
    static edgeIntersectionArrayBoundsTree(integerEdges, log) {
        ...
        const boundsTree = [];
        const hasHorizontalTree = [];
        const hasVerticalTree = [];
        ...
        const handleSelfIntersection = (level, index) => {
        ...
        const handleCrossIntersection = (level, indexA, indexB) => {
    ...
    static edgeIntersectionQuadratic(integerEdges, log) {
...
class BoundsTreeNode {
    constructor(bounds, hasHorizontal, hasVertical) {
        this.bounds = bounds;
        this.hasHorizontal = hasHorizontal;
        this.hasVertical = hasVertical;
    }
    static fromIntegerEdges(integerEdges) {
        if (integerEdges.length === 0) {
            throw new Error();
        }
        let nodes = integerEdges.map(edge => new BoundsTreeLeaf(edge));
...
//# sourceMappingURL=LineIntersector.js.map