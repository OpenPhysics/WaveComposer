...
export declare class RenderImage extends RenderProgram {
    readonly transform: Matrix3;
    readonly image: RenderImageable;
    readonly extendX: RenderExtend;
    readonly extendY: RenderExtend;
    readonly resampleType: RenderResampleType;
    readonly logic: RenderImageLogic;
    constructor(transform: Matrix3, image: RenderImageable, extendX: RenderExtend, extendY: RenderExtend, resampleType: RenderResampleType, logic?: RenderImageLogic);
    getName(): string;
     ...
     */
    static evaluateAnalyticFilter(logic: RenderImageLogic, context: RenderEvaluationContext, inverseTransform: Matrix3, minExpand: number, maxExpand: number, boundsShift: number, evaluateFace: (face: ClippableFace, x: number, y: number, px: number, py: number, area: number) => number, evaluateFull: (x: number, y: number, px: number, py: number) => number): Vector4;
    static extend(extend: RenderExtend, t: number): number;
    static extendInteger(i: number, size: number, extend: RenderExtend): number;
    serialize(): SerializedRenderImage;
    static serializeRenderImageable(imageable: RenderImageable): SerializedRenderImageable;
    static deserializeRenderImageable(obj: SerializedRenderImageable): RenderImageable;
}
export declare class RenderImageLogic {
    readonly transform: Matrix3;
    readonly image: RenderImageable;
    readonly extendX: RenderExtend;
    readonly extendY: RenderExtend;
    readonly resampleType: RenderResampleType;
    readonly inverseTransform: Matrix3;
    readonly inverseTransformWithHalfOffset: Matrix3;
    constructor(transform: Matrix3, image: RenderImageable, extendX: RenderExtend, extendY: RenderExtend, resampleType: RenderResampleType);
    equals(other: RenderImageLogic): boolean;
    evaluate(context: RenderEvaluationContext): Vector4;
}
export declare class RenderInstructionImage extends RenderInstruction {
    readonly logic: RenderImageLogic;
    constructor(logic: RenderImageLogic);
    toString(): string;
