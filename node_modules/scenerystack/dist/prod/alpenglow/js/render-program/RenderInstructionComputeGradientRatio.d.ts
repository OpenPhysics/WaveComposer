/**
 * Computes the gradient ratio for a linear or radial gradient
 *
 * @author Jonathan Olson <jonathan.olson@colorado.edu>
 */
import '../../../globals.js';
import { RenderInstruction, RenderInstructionLocation } from './RenderInstruction.js';
import type { RenderLinearGradientLogic } from './RenderLinearGradientLogic.js';
import type { RenderRadialGradientLogic } from './RenderRadialGradientLogic.js';
import type { RenderExecutionStack } from './RenderExecutionStack.js';
import type { RenderEvaluationContext } from './RenderEvaluationContext.js';
import type { RenderExecutor } from './RenderExecutor.js';
import type { ByteEncoder } from '../webgpu/compute/ByteEncoder.js';
export declare class RenderInstructionComputeGradientRatio extends RenderInstruction {
    readonly logic: RenderLinearGradientLogic | RenderRadialGradientLogic;
    readonly stopLocations: RenderInstructionLocation[];
    readonly blendLocation: RenderInstructionLocation;
    constructor(logic: RenderLinearGradientLogic | RenderRadialGradientLogic, stopLocations: RenderInstructionLocation[], blendLocation: RenderInstructionLocation);
    toString(): string;
    equals(other: RenderInstruction, areLocationsEqual: (a: RenderInstructionLocation, b: RenderInstructionLocation) => boolean): boolean;
    execute(stack: RenderExecutionStack, context: RenderEvaluationContext, executor: RenderExecutor): void;
    writeBinary(encoder: ByteEncoder, getOffset: (location: RenderInstructionLocation) => number): void;
    getBinaryLength(): number;
}
//# sourceMappingURL=RenderInstructionComputeGradientRatio.d.ts.map