...
export const binaryToRenderInstructionComputeBlendRatio = (encoder, offset, getLocation) => {
    ...
    if ((first & 0xff) === RenderInstruction.ComputeLinearBlendRatioCode) {
        const scaledNormal = new Vector2(encoder.fullF32Array[offset + 4], encoder.fullF32Array[offset + 5]);
        const off = encoder.fullF32Array[offset + 6];
        return new RenderInstructionComputeBlendRatio(new RenderLinearBlendLogic(scaledNormal, off, accuracy), zeroLocation, oneLocation, blendLocation);
    }
    else {
        const inverseTransform = Matrix3.rowMajor(encoder.fullF32Array[offset + 4], encoder.fullF32Array[offset + 5], encoder.fullF32Array[offset + 6], encoder.fullF32Array[offset + 7], encoder.fullF32Array[offset + 8], encoder.fullF32Array[offset + 9], 0, 0, 1);
        const radius0 = encoder.fullF32Array[offset + 10];
        const radius1 = encoder.fullF32Array[offset + 11];
        return new RenderInstructionComputeBlendRatio(new RenderRadialBlendLogic(inverseTransform, radius0, radius1, accuracy), zeroLocation, oneLocation, blendLocation);
    }
};
//# sourceMappingURL=binaryToRenderInstructionComputeBlendRatio.js.map