...
export class RenderLinearRange {
    ...
    static getGradientLinearRanges(min, max, offset, extend, stops) {
        const unitRanges = [];
        if (stops[0].ratio > 0) {
            unitRanges.push(new RenderLinearRange(0, stops[0].ratio, stops[0].program, stops[0].program));
        }
        for (let i = 0; i < stops.length - 1; i++) {
            ...
            if (start.ratio !== end.ratio) {
                unitRanges.push(new RenderLinearRange(start.ratio, end.ratio, start.program, end.program));
            }
        }
        if (stops[stops.length - 1].ratio < 1) {
            unitRanges.push(new RenderLinearRange(stops[stops.length - 1].ratio, 1, stops[stops.length - 1].program, stops[stops.length - 1].program));
        }
        const reversedUnitRanges = unitRanges.map(r => r.getUnitReversed()).reverse();
        // Our used linear ranges
        const linearRanges = [];
        const addSectionRanges = (sectionOffset, reversed) => {
        ...
        if (extend === RenderExtend.Pad) {
            if (min < 0) {
                ...
                linearRanges.push(new RenderLinearRange(Number.NEGATIVE_INFINITY, offset, firstProgram, firstProgram));
            }
            if (min <= 1 && max >= 0) {
            ...
            if (max > 1) {
                ...
                linearRanges.push(new RenderLinearRange(1 + offset, Number.POSITIVE_INFINITY, lastProgram, lastProgram));
            }
        }
        else {
        ...
        for (let i = 0; i < linearRanges.length - 1; i++) {
            ...
            if (range.startProgram === range.endProgram && nextRange.startProgram === nextRange.endProgram && range.startProgram === nextRange.startProgram) {
                linearRanges.splice(i, 2, new RenderLinearRange(range.start, nextRange.end, range.startProgram, range.startProgram));
                i--;
            }
        }
        return linearRanges;
...
//# sourceMappingURL=RenderLinearRange.js.map