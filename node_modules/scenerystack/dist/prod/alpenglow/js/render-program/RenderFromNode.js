...
const combine = (a, b) => new RenderBlendCompose(RenderComposeType.Over, RenderBlendType.Normal, a, b);
...
//   ]
// );
const shapeToRenderPath = (shape) => new RenderPath('nonzero', shape.subpaths.map(subpath => {
    return subpath.toPiecewiseLinear(piecewiseOptions).segments.map(line => {
        return line.start;
    });
}));
const shapesToRenderPath = (shapes) => new RenderPath('nonzero', shapes.flatMap(shape => shapeToRenderPath(shape).subpaths));
const renderPathPaintToRenderProgram = (renderPath, paint, matrix) => {
    ...
    if (paint instanceof Color) {
        return RenderPathBoolean.fromInside(renderPath, new RenderColor(colorFromTColor(paint)));
    }
    else {
        const paintMatrix = paint.transformMatrix ? matrix.timesMatrix(paint.transformMatrix) : matrix;
        if (paint instanceof LinearGradient) {
            return RenderPathBoolean.fromInside(renderPath, new RenderLinearGradient(paintMatrix, paint.start, paint.end, paint.stops.map(stop => {
                return new RenderGradientStop(stop.ratio, new RenderColor(colorFromTColor(stop.color)));
            }), RenderExtend.Pad, RenderLinearGradientAccuracy.SplitAccurate));
        }
        else if (paint instanceof RadialGradient) {
            return RenderPathBoolean.fromInside(renderPath, new RenderRadialGradient(paintMatrix, paint.start, paint.startRadius, paint.end, paint.endRadius, paint.stops.map(stop => {
                return new RenderGradientStop(stop.ratio, new RenderColor(colorFromTColor(stop.color)));
            }), RenderExtend.Pad, RenderRadialGradientAccuracy.SplitAccurate));
        }
        else if (paint instanceof Pattern) {
            return RenderPathBoolean.fromInside(renderPath, new RenderImage(paintMatrix, imagelikeToRenderImageable(paint.image), RenderExtend.Repeat, RenderExtend.Repeat, resampleType));
        }
    }
    // If unimplemented
...
const imagelikeToRenderImageable = (imagelike) => {
...
const colorFromTColor = (paint) => {
    ...
    return RenderColor.premultiply(new Vector4(paint.red / 255, paint.green / 255, paint.blue / 255, paint.alpha));
};
export class RenderFromNode {
    static nodeToRenderProgram(node, matrix = Matrix3.IDENTITY) {
        if (!node.visible) {
            return RenderColor.TRANSPARENT;
        }
        const stackPrograms = [];
        ...
        if (node instanceof Path) {
            ...
            const font = (node._font.weight === 'bold' ? self.sceneryBoldFont : self.sceneryFont);
            ...
            const flipMatrix = Matrix3.rowMajor(1, 0, 0, 0, -1, 0, // vertical flip
            0, 0, 1);
            if (shapedText) {
                const glyphShapes = [];
                let x = 0;
            ...
            if (nodeImage) {
                ...
                const renderImage = RenderPathBoolean.fromInside(renderPath, new RenderImage(matrix, imagelikeToRenderImageable(node.image), RenderExtend.Pad, RenderExtend.Pad, resampleType));
        ...
        let result = new RenderStack(stackPrograms).simplified();
        ...
        if (node.clipArea && !result.isFullyTransparent) {
            result = new RenderBlendCompose(RenderComposeType.In, RenderBlendType.Normal, result, new RenderPathBoolean(shapeToRenderPath(node.clipArea), new RenderColor(new Vector4(1, 1, 1, 1)), RenderColor.TRANSPARENT));
        }
        return result.simplified();
    }
    static addBackgroundColor(renderProgram, color) {
        return combine(renderProgram, new RenderColor(colorFromTColor(color)));
    }
    static showSim() {
        const phet = 'phet';
        const display = window[phet].joist.display;
        ...
        const sizedProgram = program.transformed(Matrix3.scaling(self.devicePixelRatio));
        const width = display.width * self.devicePixelRatio;
        const height = display.height * self.devicePixelRatio;
        ...
        canvas.style.width = `${canvas.width / self.devicePixelRatio}px`;
        canvas.style.height = `${canvas.height / self.devicePixelRatio}px`;
    ...
    static nodeToJSON(node) {
        ...
        if (addBackground) {
            program = combine(program, new RenderImage(Matrix3.scaling(5), {
                width: 2,
                height: 2,
                isFullyOpaque: true,
                evaluate: (x, y) => {
            ...
            }, RenderExtend.Repeat, RenderExtend.Repeat, RenderResampleType.NearestNeighbor));
        }
        const obj = program.simplified().serialize();
        return pretty ? JSON.stringify(obj, null, 2) : JSON.stringify(obj);
    }
    static premultipliedSRGBToColor(premultiplied) {
        const sRGB = RenderColor.unpremultiply(premultiplied);
        return new Color(sRGB.x * 255, sRGB.y * 255, sRGB.z * 255, sRGB.w);
    }
    static colorFrom(...args) {
        // @ts-expect-error We're passing Color's constructor arguments in
        const color = new Color(...args);
        return new RenderColor(new Vector4(color.red / 255, color.green / 255, color.blue / 255, color.alpha));
    }
}
//# sourceMappingURL=RenderFromNode.js.map