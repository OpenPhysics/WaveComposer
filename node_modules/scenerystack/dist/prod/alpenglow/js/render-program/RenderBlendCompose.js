...
export class RenderBlendCompose extends RenderProgram {
    constructor(composeType, blendType, a, b, logic) {
        ...
        super([a, b], isFullyTransparent, isFullyOpaque);
    ...
    static blendCompose(a, b, composeType, blendType) {
        let blended;
        if (blendType === RenderBlendType.Normal) {
            ...
            switch (blendType) {
                ...
                case RenderBlendType.Darken: {
                    c3 = new Vector3(Math.min(b3.x, a3.x), Math.min(b3.y, a3.y), Math.min(b3.z, a3.z));
                    break;
                }
                case RenderBlendType.Lighten: {
                    c3 = new Vector3(Math.max(b3.x, a3.x), Math.max(b3.y, a3.y), Math.max(b3.z, a3.z));
                    break;
                }
                case RenderBlendType.ColorDodge: {
                    c3 = new Vector3(RenderBlendCompose.colorDodge(b3.x, a3.x), RenderBlendCompose.colorDodge(b3.y, a3.y), RenderBlendCompose.colorDodge(b3.z, a3.z));
                    break;
                }
                case RenderBlendType.ColorBurn: {
                    c3 = new Vector3(RenderBlendCompose.colorBurn(b3.x, a3.x), RenderBlendCompose.colorBurn(b3.y, a3.y), RenderBlendCompose.colorBurn(b3.z, a3.z));
                    break;
                }
                ...
                case RenderBlendType.Difference: {
                    c3 = new Vector3(Math.abs(b3.x - a3.x), Math.abs(b3.y - a3.y), Math.abs(b3.z - a3.z));
                    break;
                }
        ...
        switch (composeType) {
            ...
            case RenderComposeType.Plus:
                ...
                break;
            case RenderComposeType.PlusLighter:
                return new Vector4(Math.min(a.x + b.x, 1), Math.min(a.y + b.y, 1), Math.min(a.z + b.z, 1), Math.min(a.w + b.w, 1));
            default:
                throw new Error('unimplemented composeType');
        }
        // NO clamping, because of color spaces and filters with negative lobes
        return new Vector4(fa * blended.x + fb * b.x, fa * blended.y + fb * b.y, fa * blended.z + fb * b.z, fa * a.w + fb * b.w);
    }
    static screen(cb, cs) {
        return cb.plus(cs).minus(cb.componentTimes(cs));
    }
    static colorDodge(cb, cs) {
        if (cb === 0) {
            return 0;
        }
        else if (cs === 1) {
    ...
    static hardLight(cb, cs) {
        ...
        return new Vector3(cs.x <= 0.5 ? trueOption.x : falseOption.x, cs.y <= 0.5 ? trueOption.y : falseOption.y, cs.z <= 0.5 ? trueOption.z : falseOption.z);
    }
    // TODO: check examples, tests
    static softLight(cb, cs) {
        const d = new Vector3(cb.x <= 0.25 ? (16 * cb.x - 12) * cb.x + 4 : Math.sqrt(cb.x), cb.y <= 0.25 ? (16 * cb.y - 12) * cb.y + 4 : Math.sqrt(cb.y), cb.z <= 0.25 ? (16 * cb.z - 12) * cb.z + 4 : Math.sqrt(cb.z));
        return new Vector3(cs.x > 0.5 ? cb.x + (2 * cs.x - 1) * (d.x - cb.x) : cb.x - (1 - 2 * cs.x) * cb.x * (1 - cb.x), cs.y > 0.5 ? cb.y + (2 * cs.y - 1) * (d.y - cb.y) : cb.y - (1 - 2 * cs.y) * cb.y * (1 - cb.y), cs.z > 0.5 ? cb.z + (2 * cs.z - 1) * (d.z - cb.z) : cb.z - (1 - 2 * cs.z) * cb.z * (1 - cb.z));
    }
    // TODO: check examples, tests
    static sat(c) {
...
export class RenderBlendComposeLogic {
    constructor(composeType, blendType) {
        this.composeType = composeType;
        this.blendType = blendType;
    }
    equals(other) {
...
export class RenderInstructionBlendCompose extends RenderInstruction {
    ...
    writeBinary(encoder, getOffset) {
        encoder.pushU32(RenderInstruction.BlendComposeCode | // 8 bits
            (this.logic.composeType << 8) | // 3 bits
            (this.logic.blendType << 11) // 4 bits
        ); // 0
    }
    static fromBinary(encoder, offset, getLocation) {
        const composeType = (encoder.fullU32Array[offset] >> 8) & 0x7;
        const blendType = (encoder.fullU32Array[offset] >> 11) & 0xf;
...
//# sourceMappingURL=RenderBlendCompose.js.map