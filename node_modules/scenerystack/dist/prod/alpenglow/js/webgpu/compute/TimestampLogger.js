...
export class TimestampLogger {
    // TODO: custom mark/measure like self.performance. Allow grouped/nested(!), add visual display?
    constructor(
    ...
    getGPUComputePassTimestampWrites(name) {
        ...
        if (this.querySet && this.queryBuffer) {
            ...
            return new Promise((resolve, reject) => {
                bufferLogger.withBuffer(encoder, buffer, async (arrayBuffer) => {
                    const absoluteTimestamps = [...new BigInt64Array(arrayBuffer)].slice(0, this.index);
                    const relativeTimestamps = absoluteTimestamps.map(timestamp => Number(timestamp - absoluteTimestamps[0]));
                    const result = new TimestampLoggerResult(relativeTimestamps, this.timestampNames);
                    this.index = 0;
                    this.timestampNames = [];
                    resolve(result);
                });
...
export class TimestampLoggerResult {
    constructor(timestamps, timestampNames) {
        this.timestamps = timestamps;
        this.timestampNames = timestampNames;
        this.deltas = timestamps.slice(1).map((timestamp, i) => timestamp - timestamps[i]);
    }
    toString() {
        const numToTimestamp = (n) => {
            let result = '';
    ...
    static averageTimestamps(results) {
        if (results.length === 0) {
...
//# sourceMappingURL=TimestampLogger.js.map