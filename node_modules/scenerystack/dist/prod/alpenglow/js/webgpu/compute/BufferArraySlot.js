// Copyright 2023-2025, University of Colorado Boulder
/**
 * TODO: doc
 *
 * @author Jonathan Olson <jonathan.olson@colorado.edu>
 */
import '../../../../globals.js';
import { BufferSlot } from './BufferSlot.js';
// TODO: potential labels for buffer slots? (would need to be combined when buffer slots are shared)
export class BufferArraySlot extends BufferSlot {
    constructor(concreteArrayType) {
        super(concreteArrayType);
        this.concreteArrayType = concreteArrayType;
    }
    get length() {
        return this.concreteArrayType.length;
    }
}
//# sourceMappingURL=BufferArraySlot.js.map