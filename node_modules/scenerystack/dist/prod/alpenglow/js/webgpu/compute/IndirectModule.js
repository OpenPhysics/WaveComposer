// Copyright 2023-2025, University of Colorado Boulder
/**
 * TODO: doc
 *
 * @author Jonathan Olson <jonathan.olson@colorado.edu>
 */
import '../../../../globals.js';
import { optionize3 } from '../../../../phet-core/js/optionize.js';
import { PIPELINE_BLUEPRINT_DEFAULTS, PipelineBlueprint } from './PipelineBlueprint.js';
import { Module } from './Module.js';
export const INDIRECT_MODULE_DEFAULTS = {
    // eslint-disable-next-line phet/no-object-spread-on-non-literals
    ...PIPELINE_BLUEPRINT_DEFAULTS
};
export class IndirectModule extends Module {
    constructor(providedOptions) {
        const options = optionize3()({}, INDIRECT_MODULE_DEFAULTS, providedOptions);
        const pipelineBlueprint = new PipelineBlueprint(options);
        options.create(pipelineBlueprint);
        const dispatch = (context, indirectBuffer, indirectOffset = 0) => {
            context.dispatchIndirect(pipelineBlueprint, indirectBuffer, indirectOffset);
        };
        super([pipelineBlueprint], (context, data) => {
            options.execute(context, dispatch, data);
        });
    }
}
//# sourceMappingURL=IndirectModule.js.map