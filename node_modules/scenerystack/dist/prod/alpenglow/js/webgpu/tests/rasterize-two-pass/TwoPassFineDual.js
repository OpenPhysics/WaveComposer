...
export const evaluateTwoPassFineDual = async (name, deviceContext) => {
    const filterType = PolygonFilterType.Bilinear;
    const filterScale = 50; // 25 box, 17 bilinear (comparison)
    ...
    const rasterSize = Math.ceil(outputSize * self.devicePixelRatio);
    ...
    const program = new RenderStack([
        new RenderPathBoolean(RenderPath.fromBounds(new Bounds2(0, 0, 128, 256)), convertColorSpace(new RenderColor(new Vector4(0, 0, 0, 1)), RenderColorSpace.sRGB, clientSpace), convertColorSpace(new RenderColor(new Vector4(1, 1, 1, 1)), RenderColorSpace.sRGB, clientSpace)),
        RenderPathBoolean.fromInside(new RenderPath('nonzero', smallerFace.toPolygonalFace().polygons), convertColorSpace(new RenderColor(new Vector4(1, 1, 1, 1)), RenderColorSpace.sRGB, clientSpace)),
        RenderPathBoolean.fromInside(new RenderPath('nonzero', mainFace.toPolygonalFace().polygons), convertColorSpace(new RenderLinearBlend(new Vector2(1 / 256, 0), 0, RenderLinearBlendAccuracy.Accurate, convertColorSpace(new RenderColor(new Vector4(1, 0, 0, 1)), RenderColorSpace.sRGB, RenderColorSpace.premultipliedOklab), convertColorSpace(new RenderColor(new Vector4(0.5, 0, 1, 1)), RenderColorSpace.sRGB, RenderColorSpace.premultipliedOklab)), RenderColorSpace.premultipliedOklab, clientSpace))
    ...
    const wrapBlitModule = new CompositeModule([mainModule], (context, data) => {
    ...
    const routine = await Routine.create(deviceContext, wrapBlitModule, [configSlot, coarseRenderableFacesSlot, coarseEdgesSlot, renderProgramInstructionsSlot], Routine.INDIVIDUAL_LAYOUT_STRATEGY, async (context, execute, input) => {
    ...
    canvas.style.width = `${rasterWidth / self.devicePixelRatio}px`; // TODO: hopefully integral for tests
    canvas.style.height = `${rasterHeight / self.devicePixelRatio}px`;
...
//# sourceMappingURL=TwoPassFineDual.js.map