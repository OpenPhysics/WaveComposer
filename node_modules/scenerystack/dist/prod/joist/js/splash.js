...
function positionDiv(div, splashImg) {
    ...
    const availableHeight = self.innerHeight; // eslint-disable-line phet/bad-sim-text
    const availableWidth = self.innerWidth; // eslint-disable-line phet/bad-sim-text
...
splashImage.onload = function () {
    ...
    adjustPosition();
    self.addEventListener('resize', adjustPosition);
    self.addEventListener('load', adjustPosition);
...
export const splashScreenDownloadComplete = function (brand) {
...
if (self.PHET_SPLASH_DATA_URI) {
    // For builds, this is substituted in the sim.html template.
    splashImage.src = self.PHET_SPLASH_DATA_URI;
}
else {
...
zoomEvents.forEach(zoomEvent => self.addEventListener(zoomEvent, preventZoom, {
...
export const splashScreen = {
    ...
    dispose: function () {
        ...
        if (self.PHET_SPLASH_DATA_URI) {
            delete self.PHET_SPLASH_DATA_URI;
        }
        self.removeEventListener('resize', adjustPosition);
        self.removeEventListener('load', adjustPosition);
        zoomEvents.forEach(zoomEvent => self.removeEventListener(zoomEvent, preventZoom));
        if (document.body.contains(splashBackgroundDiv)) {
            document.body.removeChild(splashBackgroundDiv);
        }
        delete self.phetSplashScreen;
    }
};
