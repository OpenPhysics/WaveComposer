...
const round = (n, places = 2) => toFixed(n, places);
...
export default class Helper {
    constructor(sim, simDisplay) {
        ...
        const measuringTapeUnitsProperty = new TinyProperty({ name: 'view units', multiplier: 0 });
        ...
        this.previewTrailProperty.link(trail => {
            ...
            if (trail) {
                ...
                if (node.bounds.isValid()) {
                    const scale = self.devicePixelRatio * 0.9 * Math.min(previewBackground.selfBounds.width / node.width, previewBackground.selfBounds.height / node.height);
                    previewNode.addChild(new Node({
                        scale: scale / self.devicePixelRatio,
        ...
        const pointerAreaTypeRadioButtonGroup = new AquaRadioButtonGroup(this.pointerAreaTypeProperty, [
            {
                ...
                createNode: (tandem) => new Text('Mouse', { fontSize: 12 })
            },
            {
                value: PointerAreaType.TOUCH,
                createNode: (tandem) => new Text('Touch', { fontSize: 12 })
            },
            {
                value: PointerAreaType.NONE,
                createNode: (tandem) => new Text('None', { fontSize: 12 })
            }
        ], {
