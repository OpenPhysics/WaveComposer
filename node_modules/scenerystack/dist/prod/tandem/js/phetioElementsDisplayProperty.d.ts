/**
 * Property that controls the view of PhET-iO Elements, predominantly in Studio.
 *
 * @author Michael Kauzmann (PhET Interactive Simulations)
 * @author Sam Reid (PhET Interactive Simulations)
 */
import '../../globals.js';
import Property from '../../axon/js/Property.js';
export declare const PhetioElementsDisplayValues: readonly ["featured", "all"];
export type PhetioElementsDisplay = (typeof PhetioElementsDisplayValues)[number];
declare const phetioElementsDisplayProperty: Property<"featured" | "all">;
export default phetioElementsDisplayProperty;
//# sourceMappingURL=phetioElementsDisplayProperty.d.ts.map