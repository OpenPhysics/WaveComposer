...
const fromByteArray = base64js.fromByteArray;
...
import DOMBlock from '../display/DOMBlock.js';
...
class Display {
    ...
    constructor(rootNode, providedOptions) {
        ...
        this._highlightVisibilityController = null;
        ...
        const options = optionize()({
            ...
            interactive: true,
            // {boolean} - If true, input event listeners will be attached to the Display's DOM element instead of the self.
    ...
    updateDisplay() {
        ...
        ;
        ;
        false // allow pending blocks / dirty
        ;
        ;
        ...
        ;
        false // make sure trails are valid
        ;
        ;
    ...
    get webglAllowed() { return this.isWebGLAllowed(); }
    getRootNode() {
     ...
     */
    setInteractiveHighlightsEnabled(interactiveHighlightsEnabled) {
        this._interactiveHighlightsEnabledProperty.value = interactiveHighlightsEnabled;
        return this;
    }
    set interactiveHighlightsEnabled(value) { this.setInteractiveHighlightsEnabled(value); }
    get interactiveHighlightsEnabled() { return this.getInteractiveHighlightsEnabled(); }
    getInteractiveHighlightsEnabled() {
    ...
    setElementCursor(cursor) {
        ...
        // full self, we can apply the workaround of controlling the body's style.
    ...
    resizeOnWindowResize() {
        const resizer = () => {
            this.setWidthHeight(self.innerWidth, self.innerHeight); // eslint-disable-line phet/bad-sim-text
        };
        self.addEventListener('resize', resizer);
        resizer();
    }
    /**
    ...
    updateOnRequestAnimationFrame(stepCallback) {
        ...
        (function step() {
            ...
            selfReference._requestAnimationFrameID = self.requestAnimationFrame(step, selfReference._domElement);
    ...
    cancelUpdateOnRequestAnimationFrame() {
        this._requestAnimationFrameID !== null && self.cancelAnimationFrame(this._requestAnimationFrameID);
    }
    /**
    ...
    initializeEvents(options) {
        ...
        const input = new Input(this, !this._listenToOnlyElement, this._batchDOMEvents, this._assumeFullWindow, this._passiveEvents, this._preventMultitouch, this._interruptMultitouch, options);
        this._input = input;
    ...
    iframeDebug() {
        ...
        iframe.width = '' + self.innerWidth; // eslint-disable-line phet/bad-sim-text
        iframe.height = '' + self.innerHeight; // eslint-disable-line phet/bad-sim-text
    ...
    foreignObjectRasterization(callback) {
        ...
        function scanForCanvases(drawable) {
            if (drawable instanceof BackboneDrawable) {
                ...
                if ((drawable instanceof CanvasBlock || drawable instanceof WebGLBlock) && drawable.canvas && drawable.canvas instanceof self.HTMLCanvasElement) {
                    addCanvas(drawable.canvas);
                }
            }
            if (DOMDrawable && drawable instanceof DOMDrawable) {
                if (drawable.domElement instanceof self.HTMLCanvasElement) {
    ...
    static elementToSVGDataURL(domElement, width, height, callback) {
        ...
        const xhtml = new self.XMLSerializer().serializeToString(domElement);
        ...
        const uint8array = new TextEncoder.TextEncoderLite('utf-8').encode(data); // eslint-disable-line no-undef
     ...
     * Returns whether this input listener is currently listening to this Display.
     */
    static hasInputListener(listener) {
...
Display.INTERRUPT_OTHER_POINTERS = (event) => {
    self.phet?.joist?.display?.interruptOtherPointers(event?.pointer);
};
