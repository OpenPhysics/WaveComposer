export default SVGSelfDrawable;
declare class SVGSelfDrawable extends SelfDrawable {
    /**
     * @public
     *
     * @param {number} renderer
     * @param {Instance} instance
     * @param {boolean} usesPaint - Effectively true if we mix in PaintableStatefulDrawable
     * @param {boolean} keepElements
     * @returns {SVGSelfDrawable}
     */
    public initialize(renderer: number, instance: Instance, usesPaint: boolean, keepElements: boolean): SVGSelfDrawable;
    usesPaint: boolean | undefined;
    keepElements: boolean | undefined;
    svgElement: any;
    svgBlock: any;
    paintState: PaintSVGState | undefined;
    /**
     * Updates the cached paints for this drawable. This should be called when the paints for the drawable have changed.
     * @protected
     */
    protected setCachedPaints(cachedPaints: any): void;
    /**
     * Called to update the visual appearance of our svgElement
     * @protected
     */
    protected updateSVG(): void;
    /**
     * to be used by our passed in options.updateSVG
     * @protected
     *
     * @param {SVGElement} element
     */
    protected updateFillStrokeStyle(element: SVGElement): void;
    /**
     * @public
     *
     * @param {SVGBlock} svgBlock
     */
    public updateSVGBlock(svgBlock: SVGBlock): void;
    defs: any;
}
import SelfDrawable from '../display/SelfDrawable.js';
import PaintSVGState from '../display/PaintSVGState.js';
//# sourceMappingURL=SVGSelfDrawable.d.ts.map