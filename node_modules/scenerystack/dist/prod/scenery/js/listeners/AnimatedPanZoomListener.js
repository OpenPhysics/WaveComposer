...
class AnimatedPanZoomListener extends PanZoomListener {
    ...
    constructor(targetNode, providedOptions) {
        ...
        if (platform.safari && !platform.mobileSafari) {
            ...
            self.addEventListener('gesturestart', boundGestureStartListener);
            // @ts-expect-error - Event type for this Safari specific event isn't available yet
            self.addEventListener('gesturechange', boundGestureChangeListener);
        }
        ...
        this.disposeAnimatedPanZoomListener = () => {
            // @ts-expect-error - Event type for this Safari specific event isn't available yet
            boundGestureStartListener && self.removeEventListener('gesturestart', boundGestureStartListener);
            // @ts-expect-error - Event type for this Safari specific event isn't available yet
            boundGestureChangeListener && self.removeEventListener('gestureChange', boundGestureChangeListener);
            if (options.stepEmitter) {
    ...
    windowKeydown(domEvent) {
        ...
        this.cancelMiddlePress();
        const simGlobal = _.get(self, 'phet.joist.sim', null); // returns null if global isn't found
...
class Wheel {
    ...
    constructor(event, targetScale) {
        ...
        if (domEvent.deltaMode === self.WheelEvent.DOM_DELTA_LINE) {
...
const isPressOrKeyboardDragListener = (obj) => {
    return typeof obj === 'object' && (obj._isPressListener ||
        obj._isKeyboardDragListener);
};
export default AnimatedPanZoomListener;
//# sourceMappingURL=AnimatedPanZoomListener.js.map