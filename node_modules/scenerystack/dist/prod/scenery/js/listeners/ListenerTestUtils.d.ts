export default ListenerTestUtils;
declare namespace ListenerTestUtils {
    /**
     * Sends a synthesized mouseDown event at the given coordinates.
     * @public
     *
     * @param {Display} display
     * @param {number} x
     * @param {number} y
     */
    function mouseDown(display: Display, x: number, y: number): void;
    /**
     * Sends a synthesized mouseUp event at the given coordinates.
     * @public
     *
     * @param {Display} display
     * @param {number} x
     * @param {number} y
     */
    function mouseUp(display: Display, x: number, y: number): void;
    /**
     * Sends a synthesized mouseMove event at the given coordinates.
     * @public
     *
     * @param {Display} display
     * @param {number} x
     * @param {number} y
     */
    function mouseMove(display: Display, x: number, y: number): void;
    /**
     * Runs a simple test with a 20x20 rectangle in a 640x480 display.
     * @public
     *
     * @param {Function} callback - Called with callback( {Display}, {Node}, {Node} ) - First node is the draggable rect
     */
    function simpleRectangleTest(callback: Function): void;
}
import Display from '../display/Display.js';
//# sourceMappingURL=ListenerTestUtils.d.ts.map