/**
 * Utilities and globals to support RichText
 *
 * @author Jonathan Olson <jonathan.olson@colorado.edu>
 */
import '../../../../globals.js';
import Text from '../../nodes/Text.js';
export type HimalayaAttribute = {
    key: string;
    value?: string;
};
export type HimalayaNode = {
    type: 'element' | 'comment' | 'text';
    innerContent: string;
};
export type HimalayaElementNode = {
    type: 'element';
    tagName: string;
    children: HimalayaNode[];
    attributes: HimalayaAttribute[];
    innerContent?: string;
} & HimalayaNode;
export declare const isHimalayaElementNode: (node: HimalayaNode) => node is HimalayaElementNode;
export type HimalayaTextNode = {
    type: 'text';
    content: string;
} & HimalayaNode;
export declare const isHimalayaTextNode: (node: HimalayaNode) => node is HimalayaTextNode;
declare const RichTextUtils: {
    scratchText: Text;
    himalayaGetAttribute(attribute: string, element: HimalayaElementNode | null): string | null;
    himalayaStyleStringToMap(styleString: string): Record<string, string>;
};
export default RichTextUtils;
//# sourceMappingURL=RichTextUtils.d.ts.map