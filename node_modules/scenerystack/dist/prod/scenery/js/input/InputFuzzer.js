...
class InputFuzzer {
    ...
    createTouchEvent(type, touches) {
        ...
        if (self.Touch !== undefined &&
            self.TouchEvent !== undefined &&
            self.Touch.length === 1 &&
            self.TouchEvent.length === 1) {
            const rawTouches = touchItems.map(touchItem => new self.Touch(touchItem));
            return new self.TouchEvent(type, {
                cancelable: true,
                bubbles: true,
                touches: rawTouches,
                targetTouches: [],
    ...
    mouseToggle() {
        ...
        domEvent.initMouseEvent(this.isMouseDown ? 'mouseup' : 'mousedown', true, true, self, 1, // click count
        this.mousePosition.x, this.mousePosition.y, this.mousePosition.x, this.mousePosition.y, false, false, false, false, 0, // button
        null);
        this.display._input.validatePointers();
        if (this.isMouseDown) {
    ...
    mouseMove() {
        ...
        // technically deprecated, but DOM4 event constructors not out yet. people on #whatwg said to use it
        domEvent.initMouseEvent('mousemove', true, true, self, 0, // click count
        this.mousePosition.x, this.mousePosition.y, this.mousePosition.x, this.mousePosition.y, false, false, false, false, 0, // button
        null);
        this.display._input.validatePointers();
...
//# sourceMappingURL=InputFuzzer.js.map