/**
 * Semi-transparent black barrier used to block input events when a dialog (or other popup) is present, and fade out
 * the background.
 *
 * @author Michael Kauzmann (PhET Interactive Simulations)
 */
import '../../globals.js';
import { ObservableArray } from '../../axon/js/createObservableArray.js';
import { EmptySelfOptions } from '../../phet-core/js/optionize.js';
import Plane, { PlaneOptions } from '../../scenery/js/nodes/Plane.js';
import { PopupableNode } from '../../sun/js/Popupable.js';
type SelfOptions = EmptySelfOptions;
export type BarrierRectangleOptions = SelfOptions & PlaneOptions;
export default class BarrierRectangle extends Plane {
    private readonly disposeBarrierRectangle;
    constructor(modalNodeStack: ObservableArray<PopupableNode>, providedOptions?: BarrierRectangleOptions);
    dispose(): void;
}
export {};
//# sourceMappingURL=BarrierRectangle.d.ts.map