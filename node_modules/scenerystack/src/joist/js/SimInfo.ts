...
class SimInfo extends PhetioObject {
  ...
  public constructor( sim: Sim ) {
    ...
    // globals
    this.putInfo( 'url', self.location.href );
    this.putInfo( 'randomSeed', self.phet.chipper.queryParameters.randomSeed );
    this.putInfo( 'userAgent', self.navigator.userAgent );
    this.putInfo( 'language', self.navigator.language );
    ...
    // @ts-expect-error - pointerEnabled isn't included in the Typescript definition of self.navigator
    if ( self.navigator.pointerEnabled ) { flags.push( 'pointerEnabled' ); }

    // @ts-expect-error - msPointerEnabled isn't included in the Typescript definition of self.navigator
    if ( self.navigator.msPointerEnabled ) { flags.push( 'msPointerEnabled' ); }
    if ( !self.navigator.onLine ) { flags.push( 'offline' ); }
    ...
    try {
      ...
      if ( ( self.devicePixelRatio || 1 ) / backingStorePixelRatio !== 1 ) { flags.push( 'pixelRatioScaling' ); }
    }
