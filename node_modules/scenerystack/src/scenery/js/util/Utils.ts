...
const Utils = {
    ...
    if ( !self.requestAnimationFrame || !self.cancelAnimationFrame ) {
      ...
      if ( !Features.requestAnimationFrame || !Features.cancelAnimationFrame ) {
        self.requestAnimationFrame = callback => {
          ...
          return self.setTimeout( () => { // eslint-disable-line phet/bad-sim-text
      ...
      // Fill in the non-prefixed names with the prefixed versions
        ...
        // @ts-expect-error
        self.requestAnimationFrame = window[ Features.requestAnimationFrame ];
        // @ts-expect-error
        self.cancelAnimationFrame = window[ Features.cancelAnimationFrame ];
      }
    }
  },
  ...
  backingScale( context: CanvasRenderingContext2D | WebGLRenderingContext ): number {
    if ( 'devicePixelRatio' in self ) {
      ...
      return self.devicePixelRatio / backingStoreRatio;
    }
    return 1;
  },
  ...
  checkWebGLSupport( extensions?: string[] ): boolean {
    ...
    try {
      // @ts-expect-error
      const gl: WebGLRenderingContext | null = !!self.WebGLRenderingContext &&
