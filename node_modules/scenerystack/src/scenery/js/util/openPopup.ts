...
/**
 * Opens the URL in a new self or tab.
 * @param url
...
function openPopup( url: string, allowPopups = allowLinksProperty.value ): void {
  ...
  if ( allowPopups && !fuzzOptOut ) {
    const popupWindow = self.open( url, '_blank' ); // open in a new window/tab

    // We can't guarantee the presence of a self object, since if it isn't opened then it will return null.
