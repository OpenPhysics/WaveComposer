...
/** @type {( Record<string, ( ob: any, style?: any ) => void ) & { pop: () => void; push: () => void; getDepth: () => number }> | null} */
export let sceneryLog = null;
...
function consoleLogFunction( ...args ) {
  ...
  self.console && self.console.log && self.console.log( ...Array.prototype.slice.call( args, 0 ) );
}
...
extend( scenery, {
  ...
  switchLogToString: function() {
    self.console && self.console.log( 'switching to string log' );
  ...
  enableIndividualLog: function( name ) {
    ...
    if ( name ) {
      ...
      sceneryLog[ name ] = sceneryLog[ name ] || function( ob, styleOverride ) {
  ...
  enableLogging: function( logNames ) {
    sceneryLog = function( ob ) { scenery.logFunction( ob ); };

    sceneryLog.push = function() {
      logPadding += '| ';
    };
    sceneryLog.pop = function() {
      logPadding = logPadding.slice( 0, -2 );
    };
    sceneryLog.getDepth = function() {
  ...
  isLoggingPerformance: function() {
    return sceneryLog.PerfCritical || sceneryLog.PerfMajor ||
           sceneryLog.PerfMinor || sceneryLog.PerfVerbose;
  }
} );
