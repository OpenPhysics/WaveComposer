...
export default class BrowserEvents {
  ...
  public static removeDisplay(
    ...
    assert && assert( _.includes( BrowserEvents.attachedDisplays, display ),
      'This display was not already attached to listen for self events' );
   ...
   *                                       used.
   */
  private static addOrRemoveListeners( element: Element | typeof self, addOrRemove: boolean, passiveEvents: boolean | null ): void {
    const forWindow = element === window;
    assert && assert( !forWindow || ( BrowserEvents.listenersAttachedToWindow > 0 ) === !addOrRemove,
      'Do not add listeners to the self when already attached, or remove listeners when none are attached' );
