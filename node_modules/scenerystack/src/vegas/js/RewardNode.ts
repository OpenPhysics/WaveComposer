...
export default class RewardNode extends CanvasNode {
  ...
  // Bounds in which to render the canvas, which represents the full self. See below for how this is computed based
  ...
  private initialize(): void {
    ...
    if ( !this.isInitialized && this.getUniqueTrail().length > 0 ) {
      ...
      // Listen to the bounds of the scene, so the canvas can be resized if the self is reshaped.
      const updateBounds = () => {
        ...
        // These bounds represent the full self relative to the scene. It is transformed by the inverse of the
